...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group03/Test_Case_08/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_08/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group03/Test_Case_08/inputrequest/request3.json	${asset_Id_Product1}
...
14. Product1 Asset2 Creation with POST Request
    [Tags]	Functional	asset	create	POST    current
    run keyword and ignore error  create Asset2 based on product1 Asset1  CreationOfRegressionProduct1Asset2_siscase1_withCol_ID.json
    ${msg}  get error message  ${response_api}
    run keyword if  '${msg}' != 'Collection cannot have assets with different states'    Fail	test1 Teardown
    log to console  ${msg}
