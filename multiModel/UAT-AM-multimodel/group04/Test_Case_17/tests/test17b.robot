...
10. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict  group04/Test_Case_17/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_17/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_17/inputrequest/request3.json	${asset_Id_Product1}
...
13. Edit Product1 Attributes
    ...
    ${response}  create Product 2 Asset1 based on product1 Asset1	EditOfAttRegressionProduct1_siscase1_withCol_ID.json
