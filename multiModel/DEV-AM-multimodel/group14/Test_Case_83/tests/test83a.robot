...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group14/Test_Case_82/inputrequest/request1.json	${asset_Id_Product2}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group14/Test_Case_82/inputrequest/request2.json	${asset_Id_Product2}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group14/Test_Case_82/inputrequest/request3.json	${asset_Id_Product2}
...
25. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group14/Test_Case_82/inputrequest/request1.json	${asset_Id_Product3}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group14/Test_Case_82/inputrequest/request2.json	${asset_Id_Product3}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group14/Test_Case_82/inputrequest/request3.json	${asset_Id_Product3}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response3.text}
    run keyword if  '${clause}' != 'False'  Fail	test1 Teardown
...
27. Link Product3 to Product 1
    [Tags]	Functional	current
    ${result}   Link Components to Asset    Link_Product3toProduct1_FU1_withoutLinkageDetails.json     ${asset_Id_Product1}
    ${SeqId}    Get Asset Link Seq_Id   ${result.json()}   ${asset_Id_Product3}
	set global variable	${assetLinkSeqId2}	${SeqId}
