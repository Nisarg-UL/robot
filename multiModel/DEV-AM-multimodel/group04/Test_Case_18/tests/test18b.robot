...
4. Validate above Asset's part of same collection
    [Tags]	Functional	Test	GET    Notcurrent
...
10. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict  group04/Test_Case_18/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_18/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_18/inputrequest/request3.json	${asset_Id_Product1}
...
13. Edit Asset1 Col_Attribute
    ...
    ${response}  create Product 2 Asset1 based on product1 Asset1	EditOfCol_AttRegressionProduct1_siscase1_withCol_ID.json    ${asset_Id_Product1}
