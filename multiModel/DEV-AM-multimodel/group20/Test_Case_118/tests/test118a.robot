...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group20/Test_Case_118/inputrequest/request1.json	${asset_Id_Product2}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group20/Test_Case_118/inputrequest/request2.json	${asset_Id_Product2}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group20/Test_Case_118/inputrequest/request3.json	${asset_Id_Product2}
...
14. Validation at End test
    [Tags]	Functional	current
    ${response}  Validate On End   ${asset_Id_Product1}
    run keyword if  '${response}' != 'Successfully validated'   Fail	test1 Teardown
