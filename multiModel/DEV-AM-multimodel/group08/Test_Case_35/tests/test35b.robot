...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group08/Test_Case_35/inputrequest/request1.json	${asset_Id_Product2}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group08/Test_Case_35/inputrequest/request2.json	${asset_Id_Product2}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group08/Test_Case_35/inputrequest/request3.json	${asset_Id_Product2}
...
13. Link Product2 to Product 1
    [Tags]	Functional	current
    run keyword and ignore error   Link Components to Asset    Link_Product1toComponent1withoutHierarchyDetails.json     ${asset_Id_Product1}
    ${error_msg}  Get Error Message  ${response_api}
    should be equal  "${error_msg}"   "Product Type not valid for Functional Use"
