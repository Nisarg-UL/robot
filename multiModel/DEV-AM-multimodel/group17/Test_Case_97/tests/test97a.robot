...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group17/Test_Case_95/inputrequest/request1.json	${asset_Id_Product2}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group17/Test_Case_95/inputrequest/request2.json	${asset_Id_Product2}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group17/Test_Case_95/inputrequest/request3.json	${asset_Id_Product2}
...
13. Link Product2 to Product 1
	...
	set global variable  ${assetLinkSeqId_2}   ${assetLinkSeqId}
...
15. Link Alternate Component to Product 2
	...
	set global variable	${assetLinkSeqId_1}	${result.json()["data"]["hasComponents"][0]["assetAssetLinkSeqId"]}
	should not be empty  ${assetLinkSeqId_1}
	log to console  ${assetLinkSeqId_1}

16. Get Component and Alternate of Asset in Collection
    [Tags]	Functional	current
    ${response}  Get Component of Asset In Collection with asset_id   ${Product_Collection_Id}  ${asset_Id_Product1}
    run keyword if  '${response}' != '${Component_ID}'    Fail 	test1 Teardown
    ${response}  Get Alternate Compoenent of Asset In Collection with asset_id   ${Product_Collection_Id}   ${asset_Id_Product1}
    run keyword if  '${response}' != '${asset_Id_Product2}'    Fail 	test1 Teardown

17. Link Alternate Component to Product 2
    [Tags]	Functional	current
    ${result}   Link Components to Asset    EditOf_Link_Alternate_Component_To_Product2.json     ${asset_Id_Product1}
	Should Be Equal As Strings	${result.json()["data"]["assetId"]}	${asset_Id_Product1}

# please check linkage atribute "Mask for AL-RTP2-FU1 Att 1" changed to value "Worst" for alternate component linkage