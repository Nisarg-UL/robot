...
6. Edit Asset1 Attribute
    [Tags]	Functional	asset	create	POST    current
    Edit product1 asset	EditOfSharedAttRegressionProduct1_siscase1_withCol_ID.json  ${asset_Id_Product1}
    ${response}  Get Asset From Endpoint   ${asset_Id_Product1}
    ${Shared_Att2_value}  Get SharedAttribute2  ${response}
    run keyword if  '${Shared_Att2_value}' != 'Shared-3'   Fail	test1 Teardown
    ${response}  Get Asset From Endpoint   ${asset_Id_Product12}
    ${Shared_Att2_value}  Get SharedAttribute2  ${response}
    run keyword if  '${Shared_Att2_value}' != 'Shared-3'   Fail	test1 Teardown
