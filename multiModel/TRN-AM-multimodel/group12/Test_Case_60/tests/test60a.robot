...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group12/Test_Case_59/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group12/Test_Case_59/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group12/Test_Case_59/inputrequest/request3.json	${asset_Id_Product1}
...
11. Complete Evaluation
	[Tags]	Functional	POST	current
	${response}  run keyword and ignore error    Complete Evaluation     markevaluationcomplete_empty.json	${asset_Id_Product1}
	${msg}  Get Error Message   ${response_api}
	run keyword if  "${msg}" != "AssessmentId can\'t be an empty string"  Fail	test1 Teardown
	log to console  ${msg}