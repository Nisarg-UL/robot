...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group22/Test_Case_134/inputrequest/request1.json	${asset_Id_Product2}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group22/Test_Case_134/inputrequest/request2.json	${asset_Id_Product2}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group22/Test_Case_134/inputrequest/request3.json	${asset_Id_Product2}
...
25. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group22/Test_Case_134/inputrequest/request1.json	${asset_Id_Product3}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group22/Test_Case_134/inputrequest/request2.json	${asset_Id_Product3}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group22/Test_Case_134/inputrequest/request3.json	${asset_Id_Product3}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response3.text}
    run keyword if  '${clause}' != 'False'  Fail	test1 Teardown
...
28. Get Component and Alternate of Asset in Collection
    [Tags]	Functional	current
    ${response}  run keyword and ignore error  Get Error Message for Get Compoenent of Asset In Collection with asset_id  ${Product_Collection_Id}  0000XXXX-XXXX-00X0-X000-00X0XXXXXXX
