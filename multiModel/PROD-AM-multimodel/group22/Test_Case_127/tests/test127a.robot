...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group22/Test_Case_127/inputrequest/request1.json	${asset_Id_Product2}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group22/Test_Case_127/inputrequest/request2.json	${asset_Id_Product2}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group22/Test_Case_127/inputrequest/request3.json	${asset_Id_Product2}
...
16. Get Component and Alternate of Asset in Collection
    ...
    run keyword and ignore error  Get Error Message for Get Compoenent of Asset In Collection   ${Product_Collection_Id}
    ${result}=  get response body
    ${msg}  Get Error Message   ${result}
