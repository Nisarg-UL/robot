...
11. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict  group4/Test_Case_14/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group4/Test_Case_14/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group4/Test_Case_14/inputrequest/request3.json	${asset_Id_Product1}
...
16. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict  group4/Test_Case_14/inputrequest/request1.json	${asset_Id_Product12}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group4/Test_Case_14/inputrequest/request2.json	${asset_Id_Product12}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group4/Test_Case_14/inputrequest/request3.json	${asset_Id_Product12}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response3.text}
    run keyword if  '${clause}' != 'False'  Fail	test1 Teardown
...
#    ${response}  create Product 2 Asset1 based on product1 Asset1	EditOfCol_AttRegressionProduct1_siscase1_withoutCol_ID.json
...
20. Get the Colletion_ID
    [Tags]	Functional	Notcurrent
    Get Collection_ID   ${asset_Id_Product1}

21. Validate Assst is part of New collection only
    [Tags]	Functional	GET    Notcurrent
    ${response}  Get Collection Asset Link  ${Collection_Id}
    run keyword if  ${response} != (('${asset_Id_Product1}',),)    Fail	test1 Teardown

22. Validate Assst is Not part of Old collection and Old collection End dated
    [Tags]	Functional	GET    Notcurrent
    ...
    run keyword if  '${response}' != '((datetime.datetime(2017, 6, 23, 15, 45, 29),), (datetime.datetime(2999, 12, 31, 23, 59, 59),))'    Fail	test1 Teardown
