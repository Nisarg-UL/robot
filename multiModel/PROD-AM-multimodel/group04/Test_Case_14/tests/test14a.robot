...
11. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict  group04/Test_Case_14/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_14/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_14/inputrequest/request3.json	${asset_Id_Product1}
...
16. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict  group04/Test_Case_14/inputrequest/request1.json	${asset_Id_Product12}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_14/inputrequest/request2.json	${asset_Id_Product12}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_14/inputrequest/request3.json	${asset_Id_Product12}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response3.text}
    run keyword if  '${clause}' != 'False'  Fail	test1 Teardown
...
19. Edit Asset1 Attribute
    [Tags]	Functional	asset	create	POST    Notcurrent
    ${response}  create Product 2 Asset1 based on product1 Asset1	EditOfAttRegressionProduct1_siscase1_withoutCol_ID.json
    run keyword if  '${response}' != 'Collection already exists with collectionId - ${Collection_Id}'    Fail	test1 Teardown
