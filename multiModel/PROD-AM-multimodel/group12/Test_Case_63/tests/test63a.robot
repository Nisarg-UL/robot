...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group12/Test_Case_63/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group12/Test_Case_63/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group12/Test_Case_63/inputrequest/request3.json	${asset_Id_Product1}
...
11. Complete Evaluation
	[Tags]	Functional	POST	current
	run keyword and ignore error    Complete Evaluation	markcollectionnotcomplete.json	${asset_Id_Product1}
	${msg}  Get Error Message   ${response_api}
	run keyword if  "${msg}" != "AssessmentId's are not passed in the request"  Fail	test1 Teardown
	log to console  ${msg}