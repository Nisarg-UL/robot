...
11. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_26/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group05/Test_Case_26/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group05/Test_Case_26/inputrequest/request3.json	${asset_Id_Product1}
...
16. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_26/inputrequest/request1.json	${asset_Id_Product12}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group05/Test_Case_26/inputrequest/request2.json	${asset_Id_Product12}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group05/Test_Case_26/inputrequest/request3.json	${asset_Id_Product12}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response3.text}
    run keyword if  '${clause}' != 'False'  Fail	test1 Teardown
...
19. Modify Asset1 Attribute
    [Tags]	Functional	asset	create	POST    current
    ${response}  Edit Product1 Asset for 400	ModifyAttRegressionProduct1_siscase1_withoutCol_ID.json     ${asset_Id_Product1}
#    run keyword if  '${response}' != 'Collection cannot have assets with different states'    Fail	test1 Teardown
    run keyword if  '${response}' != 'Collection already exists with collectionId - ${product_collection_id}'    Fail	test1 Teardown
