...
34. View Private Label Assets of a modified base asset
    [Tags]	Functional	certificate	PL  POST    current
...
35. Get Private Label Impact Details for a Base Certificate
    [Tags]	Functional	certificate	PL  create	POST    current
...
36. Unlink Impacted Asset from Private Label and Link the latest asset to Private Label
	[Tags]	Functional	POST	current
    ${result}  run keyword and ignore error   Update Impacted model for Private Label   Unlink_Asset_Template.json   ${PrivateLabel_Asset_Id1}
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'The pl asset id is not impacted'  Fail	test1 Teardown
	log to console  ${msg}