...
40. Unlink Impacted Asset from Private Label and Link the latest asset to Private Label
	[Tags]	Functional	POST	current
    ${result}  run keyword and ignore error   Update Impacted model for Private Label   Unlink_Asset_Template.json   12345
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'Invalid pl asset id'  Fail	test1 Teardown
	log to console  ${msg}