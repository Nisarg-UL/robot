...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group04/Test_Case_07/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_07/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_07/inputrequest/request3.json	${asset_Id_Product1}
...
19. Search for the Private Label
    ...
    ${response}    search private label  certificateType=Regression%20Scheme&productType=Regression%20Test%20Product%201&ownerReference=${Asset_Owner_Ref}&certificateName=Regression-US001-1
