...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group16/Test_Case_21/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group16/Test_Case_21/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group16/Test_Case_21/inputrequest/request3.json	${asset_Id_Product1}
...
#    log to console  ${response}
#    run keyword if  "${response}" != "Private label exists for base certificate. Exipre the private label prior to the base certificate"   Fail	test1 Teardown
...
21. Get Private Label certificate status
...
22. Get Private Label certificate status
    [Tags]	Functional	PL	current
