...
19. Get Private Label Details for a Base Certificate
    [Tags]	Functional	certificate	PL  create	POST    current
    ${result}  Get Private Label Details of a base Certificate  Regression-US001-1  Regression%20Scheme    ${Cert_Owner_Ref}
...
22. Certify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Certify_PrivateLabelCertificatewithWDfuturetoCD&EDfuturetoWD.json
...
34. Modify base Certificate
	[Tags]	Functional	certificate	create	POST    current
    Modify Certificate    Certificate/Modify_RegressionSchemeCertificate.json
...
39. Get Private Label Impact Details for a Base Certificate
    [Tags]	Functional	certificate	PL  create	POST    current
    ${result}  Get Private Label Impact Details of a base Certificate    certificateName=Regression-US001-1&&certificateType=Regression%20Scheme&&ownerReference=${cert_owner_ref}
    run keyword if  '${result}' != 'Y'     Fail   test1 Teardown

40. Certificate Date Validation
    [Tags]	Functional	certificate	PL  create	POST    current
    Certificate Date Validation    Private_Label/Certificate_EDpriortoPLED&WDpriortoPLWD.json   certificateName=Regression-US001-1&&certificateType=Regression%20Scheme&&ownerReference=${cert_owner_ref}
	run keyword if  '${impacted_expiry_date}' != '3999-01-01'  Fail	test1 Teardown
	run keyword if  '${impacted_withdrawal_date}' != '2999-01-01'  Fail	test1 Teardown
