...
18. Get Base Certificate Details for a Private Label
    [Tags]	Functional	certificate	PL  create	POST    current
    ${result}  Get Base Certificate Details for a Private Label   ${PrivateLabel_Id}
    run keyword if  ${result} != "${Cert_Owner_Ref}"     Fail   test1 Teardown
    log to console  Certificate Owner Reference: ${result}