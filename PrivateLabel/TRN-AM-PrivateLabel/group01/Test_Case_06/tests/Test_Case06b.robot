...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group01/Test_Case_06/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group01/Test_Case_06/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group01/Test_Case_06/inputrequest/request3.json	${asset_Id_Product1}
...
18. Create Private Label
    [Tags]	Functional	certificate	PL  create	POST    current
    run keyword and ignore error  Create private label    Private_Label/CreationOfPrivateLabel_RegressionSchemeCertificate.json
    ${response}  Get Error Message  ${response_api}
    log to console  ${response}
    run keyword if  '${response}' != 'Base certificate is either expired or withdrawn or not active.'   Fail	test1 Teardown