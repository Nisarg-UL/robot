...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group16/Test_Case_09/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group16/Test_Case_09/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group16/Test_Case_09/inputrequest/request3.json	${asset_Id_Product1}
...
21. Certify Private Label with WD = <empty> and ED < System's date(now())
    [Tags]	Functional	certificate	PL  POST    current
    run keyword and ignore error  Certify Private Label  Private_Label/Certify_PrivateLabelCertificatewithWDempty&EDpriortoCD_withIDequaltoPastdate.json
