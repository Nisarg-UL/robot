...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group16/Test_Case_13/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group16/Test_Case_13/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group16/Test_Case_13/inputrequest/request3.json	${asset_Id_Product1}
...
21. Certify Private Label with WD > System's date(now()) and ED < Withdrawn Date and ED < System's date(Now())
    [Tags]	Functional	certificate	PL  POST    current
    run keyword and ignore error  Certify Private Label  Private_Label/Certify_PLCertwithWDfuturetoCD&EDpriortoWD&EDpriortoCD_withIDequaltoPastDate.json
    ${response}  Get Error Message  ${response_api}
    log to console  ${response}
    run keyword if  "${response}" != "Private labels issue date should be greater than or equal to the base certificates issue date"   Fail	test1 Teardown
