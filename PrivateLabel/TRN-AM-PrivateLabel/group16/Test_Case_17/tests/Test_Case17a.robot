...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group16/Test_Case_16/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group16/Test_Case_16/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group16/Test_Case_16/inputrequest/request3.json	${asset_Id_Product1}
...
21. Certify Private Label (Withdraw)
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Certify_PrivateLabelCertificatewithWDpriortoCD&EDempty.json
...
27. Certify Private Label (Active)
    [Tags]	Functional	certificate	PL  POST    current
...
30. Withdrawn Base certificate
    [Tags]	Functional	certificate	create	POST    current
    set global variable  ${Certificate_Id}   ${Certificate_Id_Modify}
    run keyword and ignore error  Certify Certificate  Certificate/Certify_RegressionCertificatewithWDpriortoTodaysdate.json
    ${response}  Get Error Message  ${response_api}
    log to console  ${response}

30 Validate Private Label Status
    [Tags]	Functional	certificate PL	create	current
    ${response}  Get Private Label status   ${PrivateLabel_Id}
    log to console  ${response}
    run keyword if	${response} != "Withdrawn"	Fail	test1 Teardown