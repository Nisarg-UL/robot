...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group10/Test_Case_06/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group10/Test_Case_06/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group10/Test_Case_06/inputrequest/request3.json	${asset_Id_Product1}
...
22. View Private Label Questions Details
    [Tags]	Functional	certificate	PL  POST    current
    ${response}  View Private Label Certify Details    ${PrivateLabel_Id}
