...
31. Modify base Certificate1 for version3
	[Tags]	Functional	certificate	create	POST    current
...
34. Get Private Label details from Private Label2 Table
    [Tags]	Functional	certificate	PL  create	POST    current
...
35. Withdraw Private Label2 with with WD < System's date(now()) and ED = <empty>
    [Tags]	Functional	certificate	PL  POST    current
...
38. Modify Private Label2 for version3
    [Tags]	Functional	certificate	PL  POST    current
...
39. Get Private Label details from Private Label2 Table
    [Tags]	Functional	certificate	PL  create	POST    current
    Get PrivateLabelId from Private Label Table     ${Certificate_Id2}
...
41. Validate Certificate Status
    ...
    ${response4}  Get certificate status with certificateId   ${Certificate_Id_Modify2}
    log to console  ${response4}
    run keyword if	${response4} != "Active"	Fail	test1 Teardown

42. Validate Private Label Status
    [Tags]	Functional	certificate PL	create	current
    ${response}  Get Private Label status   ${PrivateLabel_Id}
    log to console  ${response}
    run keyword if	${response} != "Withdrawn"	Fail	test1 Teardown
    ...
    log to console  ${response1}
    run keyword if	${response1} != "Active"	Fail	test1 Teardown
    ${response2}  Get Private Label status   ${PrivateLabel2_Id_Modify}
    log to console  ${response2}
    run keyword if	${response2} != "Withdrawn"	Fail	test1 Teardown
    ${response3}  Get Private Label status   ${PrivateLabel2_Id_Modify2}
    log to console  ${response3}
