...
29. Modify Private Label1
    [Tags]	Functional	certificate	PL  POST    current
    Modify Private Lable    Private_Label/Modify_PrivateLabel2Certificate.json   ${PrivateLabel_Id}
...
37. Modify Private Label1 for version3
    [Tags]	Functional	certificate	PL  POST    current
    Modify Private Lable for Version3    Private_Label/Modify_PrivateLabel2Certificate_Revision2.json   ${PrivateLabel_Id_Modify}
    run keyword if  '${PrivateLabel_Id}' == '${PrivateLabel_Id_Modify2}'     Fail

38. Get Private Label details from Private Label1 Table
    [Tags]	Functional	certificate	PL  create	POST    current
    Get PrivateLabelId from Private Label Table     ${Certificate_Id}
    run keyword if  '${PrivateLabel_Id}, ${PrivateLabel_Id_Modify}, ${PrivateLabel_Id_Modify2}' != '${pl_Id}'     Fail
    Get Unique PrivateLabelId from Private Label Table   ${PrivateLabel_Id_Modify}
    run keyword if  '${Unique_PrivateLabel_Id}' == '${PrivateLabel_Id_Modify2}'     Fail
    Get Private Label Version from Private Label Table   ${PrivateLabel_Id_Modify2}
    run keyword if  '${PrivateLabel_Ver}' != '3.0'     Fail

39. Reinstate Private Label1
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Certify_PrivateLabelCertificate_Version3.json

40. Modify Private Label2 for version3
    [Tags]	Functional	certificate	PL  POST    current
    Modify Private Lable2 for Version3    Private_Label/Modify_PrivateLabelCertificate2_Revision2.json   ${PrivateLabel2_Id_Modify}
    run keyword if  '${PrivateLabel2_Id_Modify}' == '${PrivateLabel2_Id_Modify2}'     Fail

41. Get Private Label details from Private Label2 Table
    [Tags]	Functional	certificate	PL  create	POST    current
    Get PrivateLabelId from Private Label Table     ${Certificate_Id2}
    run keyword if  '${PrivateLabel_Id2}, ${PrivateLabel2_Id_Modify}, ${PrivateLabel2_Id_Modify2}' != '${pl_Id}'     Fail
    Get Unique PrivateLabelId from Private Label Table   ${PrivateLabel2_Id_Modify2}
    run keyword if  '${Unique_PrivateLabel_Id}' == '${PrivateLabel2_Id_Modify2}'     Fail
    Get Private Label Version from Private Label Table   ${PrivateLabel2_Id_Modify2}
    run keyword if  '${PrivateLabel_Ver}' != '3.0'     Fail

42. Reinstate Private Label2
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Certify_PrivateLabelCertificate2_Version3.json

43. Validate Private Label Status
    [Tags]	Functional	certificate PL	create	current
    ...
    log to console  ${response1}
    run keyword if	${response1} != "Active"	Fail	test1 Teardown
    ${response2}  Get Private Label status   ${PrivateLabel_Id_Modify}
    log to console  ${response2}
    run keyword if	${response2} != "Withdrawn"	Fail	test1 Teardown
    ${response3}  Get Private Label status   ${PrivateLabel2_Id_Modify}
    log to console  ${response3}
    run keyword if	${response3} != "Withdrawn"	Fail	test1 Teardown
    ${response4}  Get Private Label status   ${PrivateLabel_Id_Modify2}
    log to console  ${response4}
    run keyword if	${response4} != "Active"	Fail	test1 Teardown
    ${response5}  Get Private Label status   ${PrivateLabel2_Id_Modify2}
    log to console  ${response5}
    run keyword if	${response5} != "Active"	Fail	test1 Teardown

44. Validate Certificate Status
    [Tags]	Functional	certificate	create	current
    ${response1}  Get certificate status with certificateId   ${Certificate_Id}
    log to console  ${response1}
    run keyword if	${response1} != "Active"	Fail	test1 Teardown
