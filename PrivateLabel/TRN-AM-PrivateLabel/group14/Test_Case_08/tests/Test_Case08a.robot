...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group14/Test_Case_04/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group14/Test_Case_04/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group14/Test_Case_04/inputrequest/request3.json	${asset_Id_Product1}
...
22. Modify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Modify Private Lable    Private_Label/Modify_PrivateLabelCertificate.json    ${PrivateLabel_Id}

# In this case when we try to modify PL with revision number = 1
#{"apiversion":"5.0","code":400,"status":"Bad Request","message":"Private label already exists with same name for the given certificateType and ownerReference."}
#Please investigate later
#
#
#22. Certify Private Label
#    [Tags]	Functional	certificate	PL  POST    current
#    Certify Private Label  Private_Label/Update_Certify_PrivateLabelCertificate.json
