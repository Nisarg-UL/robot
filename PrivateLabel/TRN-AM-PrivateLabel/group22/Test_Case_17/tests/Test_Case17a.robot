...
20. Edit Private Label with Invalid Private Label
    [Tags]	Functional	certificate	PL  Edit    POST    current
    ${response}     run keyword and ignore error    Edit Private Label    Private_Label/EditOfPrivateLabel_RegressionSchemeCertificate_Cert_Name.json    12345
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Value doesn\'t exist in DB for this private Label Id"  Fail	test1 Teardown
	log to console  ${msg}
