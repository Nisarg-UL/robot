...
20. Edit Private Label using Wrong Data Type
    [Tags]	Functional	certificate	PL  Edit    POST    current
    ${response}     run keyword and ignore error    Edit Private Label    Private_Label/EditOfPrivateLabel_RegressionSchemeCertificate_withWrongDataType.json    ${PrivateLabel_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Exception occurred while validating the privateLabel - Error while validating attribute- Energy Efficiency Rating -Data Type- decimal"  Fail	test1 Teardown
	log to console  ${msg}
