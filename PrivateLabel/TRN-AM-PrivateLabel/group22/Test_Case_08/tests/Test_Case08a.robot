...
20. Edit Private Label
    [Tags]	Functional	certificate	PL  Edit    POST    current
    Edit Private Label    Private_Label/EditOfPrivateLabel_RegressionSchemeCertificate_Cert_Name.json   ${PrivateLabel_Id}
    run keyword if  '${PrivateLabel_Id}' != '${PrivateLabel_Id_Edit}'     Fail

21. Get Private Label details from Private Label Table
    [Tags]	Functional	certificate	PL  create	POST    current
    Get PrivateLabelId from Private Label Table     ${Certificate_Id}
    run keyword if  '${PrivateLabel_Id_Edit}' != '${pl_Id}'     Fail
    Get Unique PrivateLabelId from Private Label Table   ${PrivateLabel_Id_Edit}
    run keyword if  '${Unique_PrivateLabel_Id}' != '${PrivateLabel_Id_Edit}'     Fail
    Get Private Label Version from Private Label Table   ${PrivateLabel_Id_Edit}
    run keyword if  '${PrivateLabel_Ver}' != '1.0'     Fail
