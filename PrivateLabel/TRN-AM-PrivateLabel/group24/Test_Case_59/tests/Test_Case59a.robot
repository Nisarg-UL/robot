...
14. Edit Private Label Asset with Invalid family_Series Object
    [Tags]	Functional	certificate	PL  create	POST    current
    run keyword and ignore error     Edit Private Label Asset taxonomy and model nomenclature  Edit_PL_Asset_Invalid_family_Series_Object.json   ${PrivateLabel_Id}
    ${response}  get error message  ${response_api}
    run keyword if  "${response}" != "Input Taxonomy key(s) is missing or invalid in the input Request"	Fail	test1 Teardown
