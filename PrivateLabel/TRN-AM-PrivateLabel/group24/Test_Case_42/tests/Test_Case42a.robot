...
#    should be equal  ${UL_Asset_Id}   ["${asset_Id_Product1}", "${asset_Id_Product1}"]
    ...
    should be equal  ${pl_asset_family_series}   ["Reg-Test", "Reg-Test"]

15. Edit Private Label Asset with Empty Family_Series
    [Tags]	Functional	certificate	PL  create	POST    current
    Edit Private Label Asset taxonomy and model nomenclature  Edit_PL_Asset_Empty_Family_Series.json   ${PrivateLabel_Id}
    should be empty  ${pl_asset_edit_error}
    should not be empty   ${pl_asset_edit_success}
    ${success_pl_assetId}   get_key_values_from_list    ${pl_asset_edit_success}    ${plAssetId_key}
    should be equal  ${success_pl_assetId}   ["${PrivateLabel_Asset_Id1}"]
    ${success_assetId}   get_key_values_from_list    ${pl_asset_edit_success}    ${AssetId_key}
    should be equal  ${success_assetId}   ["${asset_Id_Product1}"]
    ${success_models}   pl_asset_edit_values    ${pl_asset_edit_success}    ${modelName_key}
    should be equal  ${success_models}   ["${pl_model_name_1}_${current_time}"]
...
#    should be equal  ${UL_Asset_Id}   ["${asset_Id_Product1}", "${asset_Id_Product1}"]
    should be equal  ${pl_asset_model_name}   ["${pl_model_name_1}_${current_time}", "PL_Regression_Test_Model_2"]
    should be equal  ${pl_asset_ref_no}   ["PL_DEMOFILE01", "PL_DEMOFILE01"]
    should be equal  ${pl_asset_family_series}   ["${empty}", "Reg-Test"]
