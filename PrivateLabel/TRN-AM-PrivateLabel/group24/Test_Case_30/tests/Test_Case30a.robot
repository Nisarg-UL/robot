...
16. Edit Multiple Private Label Asset with Duplicate Taxonomy of different Base Asset & Existing Reference Number
    [Tags]	Functional	certificate	PL  create	POST    current
    Edit Private Label Asset taxonomy and model nomenclature  Edit_Multiple_PL_Assets_DuplicateTaxonomy_OfDifferentBaseAsset_ExistingReferenceNumber_ChangeFlag_N.json   ${PrivateLabel_Id}
    ${success_models}   pl_asset_edit_values    ${pl_asset_edit_success}    ${modelName_key}
    should be equal  ${success_models}   ["PL_Regression_Test_Model_3_Edit", "PL_Regression_Test_Model_12"]
    ${success_pl_assetId}  pl_asset_edit_values    ${pl_asset_edit_success}  ${plAssetId_key}
    should be equal  ${success_pl_assetId}   ["${PrivateLabel_Asset_Id3}", "${PrivateLabel_Asset_Id2}"]
