...
16. Edit Multiple Private Label Asset with Duplicate Taxonomy of different Base Asset & New Reference Number
    [Tags]	Functional	certificate	PL  create	POST    current
    Edit Private Label Asset taxonomy and model nomenclature  Edit_Multiple_PL_Assets_DuplicateTaxonomy_OfDifferentBaseAsset_NewReferenceNumber_ChangeFlag_Y.json   ${PrivateLabel_Id}
    ${success_models}   pl_asset_edit_values    ${pl_asset_edit_success}    ${modelName_key}
    should be equal  ${success_models}   ["${pl_model_name_1}_${current_time}", "PL_Regression_Test_Model_12", "PL_Regression_Test_Model_3_Edit", "PL_Regression_Test_Model_12"]
    ${success_pl_assetId}  pl_asset_edit_values    ${pl_asset_edit_success}  ${plAssetId_key}
    should be equal  ${success_pl_assetId}   ["${PrivateLabel_Asset_Id1}", "${PrivateLabel_Asset_Id12}", "${PrivateLabel_Asset_Id3}", "${PrivateLabel_Asset_Id2}"]
    ${success_assetId}   pl_asset_edit_values    ${pl_asset_edit_success}    ${AssetId_key}
    should be equal  ${success_assetId}   ["${asset_Id_Product1}", "${asset_Id_Product12}", "${asset_Id_Product1}", "${asset_Id_Product1}"]
...
#    should be equal  ${PL_Id}   ["${PrivateLabel_Id}", "${PrivateLabel_Id}"]
    should be equal  ${PL_Asset_Id}   ["${PrivateLabel_Asset_Id1}", "${PrivateLabel_Asset_Id2}", "${PrivateLabel_Asset_Id3}", "${PrivateLabel_Asset_Id12}"]
    should be equal  ${Asset_Id}   ["${asset_Id_Product1}", "${asset_Id_Product1}", "${asset_Id_Product1}", "${asset_Id_Product12}"]
#    should be equal  ${UL_Asset_Id}   ["${asset_Id_Product1}", "${asset_Id_Product1}"]
    should be equal  ${pl_asset_model_name}   ["${pl_model_name_1}_${current_time}", "PL_Regression_Test_Model_12", "PL_Regression_Test_Model_3_Edit", "PL_Regression_Test_Model_12"]
    should be equal  ${pl_asset_ref_no}   ["PL_DEMOFILE01_Edit", "PL_DEMOFILE01_Edit", "PL_DEMOFILE01_Edit", "PL_DEMOFILE01_Edit"]