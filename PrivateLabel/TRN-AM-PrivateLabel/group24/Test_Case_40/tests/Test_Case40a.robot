...
#    should be equal  ${PL_Id}   ["${PrivateLabel_Id}", "${PrivateLabel_Id}"]
    should be equal  ${PL_Asset_Id}   ["${PrivateLabel_Asset_Id1}", "${PrivateLabel_Asset_Id2}"]
    should be equal  ${Asset_Id}   ["${asset_Id_Product1}", "${asset_Id_Product1}"]
#    should be equal  ${UL_Asset_Id}   ["${asset_Id_Product1}", "${asset_Id_Product1}"]
    should be equal  ${pl_asset_model_name}   ["${pl_model_name_1}_${current_time}", "PL_Regression_Test_Model_2"]
    should be equal  ${pl_asset_ref_no}   ["PL_DEMOFILE01", "PL_DEMOFILE01"]

15. Edit Private Label Asset with Invalid AssetId
    [Tags]	Functional	certificate	PL  create	POST    current
    Edit Private Label Asset taxonomy and model nomenclature  Edit_PL_Asset_Invalid_assetId.json   ${PrivateLabel_Id}
    ${success_models}   pl_asset_edit_values    ${pl_asset_edit_success}    ${modelName_key}
    should be equal  ${success_models}   ["PL_Regression_Test_Model_2"]
    ${success_pl_assetId}  pl_asset_edit_values    ${pl_asset_edit_success}  ${plAssetId_key}
    should be equal  ${success_pl_assetId}   ["${PrivateLabel_Asset_Id2}"]
    ...
    should be equal  ${error_messages}   ["Invalid/empty Base Asset ID passed in the taxonomy object"]
    ${error_pl_assetId}  pl_asset_edit_values    ${pl_asset_edit_error}  ${plAssetId_key}
    should be equal  ${error_pl_assetId}   ["${PrivateLabel_Asset_Id1}"]
    ${error_assetId}   pl_asset_edit_values    ${pl_asset_edit_error}    ${AssetId_key}
    should be equal  ${error_assetId}   ["ABCDEFGH-IJKL-MNOP-QRSTUVWXYZ00"]
    ${error_models}   pl_asset_edit_values    ${pl_asset_edit_error}    ${modelName_key}
    should be equal  ${error_models}   ["PL_Regression_Test_Model_1_Edit"]
...
#    should be equal  ${UL_Asset_Id}   ["${asset_Id_Product1}", "${asset_Id_Product1}"]
    should be equal  ${pl_asset_model_name}   ["${pl_model_name_1}_${current_time}", "PL_Regression_Test_Model_2"]
    should be equal  ${pl_asset_ref_no}   ["PL_DEMOFILE01_Edit", "PL_DEMOFILE01_Edit"]
