...
28. Add Party To Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Add Parties To Private Label  Private_Label/Associate_New_Brand_Name_Owner_Parties_Certificate.json     ${PrivateLabel_Id_Modify}

29. Certify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Certify_PrivateLabelCertificate_Version2.json

30. Get Private Label certificate status
    [Tags]	Functional	PL	current
    ${response}  Get Private Label status   ${PrivateLabel_Id_Modify}
    log to console  ${response}
    run keyword if	${response} != "Active"	Fail	test1 Teardown

31. Post Date CET_V2.2
	[Tags]	Functional	POST	current
	Activate CET   ${certificate_hierarchy_IdV2.2}

32. Modify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Modify Private Lable for Version3    Private_Label/Modify_PrivateLabelCertificate_Revision3.json   ${PrivateLabel_Id_Modify}
    run keyword if  '${PrivateLabel_Id_Modify}' == '${PrivateLabel_Id_Modify2}'     Fail
    run keyword if  '${certificate_hierarchy_Id_Modify}' == '${certificate_hierarchy_Id_Modify2}'     Fail

33. Get Private Label details from Private Label Table
    [Tags]	Functional	certificate	PL  create	POST    current
    Get PrivateLabelId from Private Label Table     ${Certificate_Id}
    run keyword if  '${PrivateLabel_Id}, ${PrivateLabel_Id_Modify}, ${PrivateLabel_Id_Modify2}' != '${pl_Id}'     Fail
    Get Unique PrivateLabelId from Private Label Table   ${PrivateLabel_Id_Modify2}
    should not be empty  ${Unique_PrivateLabel_Id}
...
34. End Date CET_V2.1
	[Tags]	Functional	POST	current
	Expire CET   ${certificate_hierarchy_IdV2.1}

35. End Date CET_V2.2
	[Tags]	Functional	POST	current
