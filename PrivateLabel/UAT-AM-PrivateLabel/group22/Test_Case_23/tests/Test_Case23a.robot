...
24. Modify Private Label by changing the owner reference
    [Tags]	Functional	certificate	PL  Edit    POST    current
    ${response}     run keyword and ignore error    Modify Private Label with New Owner Reference    Private_Label/Modify_PrivateLabelCertificate_changingOwnerReference.json    ${PrivateLabel_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Private label\'s certificateType or ownerReference cannot be edited"  Fail	test1 Teardown
	log to console  ${msg}
