...
25. Post Date CET_V2.2
	[Tags]	Functional	POST	current
	Activate CET   ${certificate_hierarchy_IdV2.2}

26. Modify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    ${response}  run keyword and ignore error   Modify Private Lable    Private_Label/Modify_PrivateLabelCertificateV2.2.json   ${PrivateLabel_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "The following party role(s) - Local Representative are invalid per the latest Certificate template and have been removed automatically."  Fail	test1 Teardown
	log to console  ${msg}
