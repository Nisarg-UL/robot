...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group14/Test_Case_03/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group14/Test_Case_03/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group14/Test_Case_03/inputrequest/request3.json	${asset_Id_Product1}
...
21. Certify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Add_Recommendations_To_PrivateLabelCertificate.json
...
23. Get Private Label details from Recommendations Table
    [Tags]	Functional	certificate	PL  create	POST    current
    Get Private Label Version from Recommendations Table   ${Unique_PrivateLabel_Id}
    run keyword if  '${PrivateLabel_Ver_recommendations}' != '1.0'     Fail