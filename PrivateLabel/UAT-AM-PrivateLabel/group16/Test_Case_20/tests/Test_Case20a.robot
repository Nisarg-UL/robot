...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group16/Test_Case_19/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group16/Test_Case_19/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group16/Test_Case_19/inputrequest/request3.json	${asset_Id_Product1}
...
17. Create Private Label(PL1)
    ...
    set global variable  ${PrivateLabel_Id1}    ${PrivateLabel_Id}
...
21. Expire Private Label(PL1)
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Certify_PrivateLabelCertificatewithWDempty&EDpriortoCD.json
...
23. Create another Private Label(PL2)
    [Tags]	Functional	certificate	PL  create	POST    current
    Create private label    Private_Label/CreationOfPrivateLabel2_RegressionSchemeCertificate.json
    should not be empty  ${PrivateLabel_Id}
    log to console  ${PrivateLabel_Id}

24. Add Asset To Private Label
    [Tags]	Functional	certificate	PL  POST    current
    ${response}  Add Asset To PL  Private_Label/Add_Asset_To_Private_Label.json
    set global variable  ${PrivateLabel_Asset_Id}   ${response}

25. View Private Label Asset
    [Tags]	Functional	certificate	PL  POST    current
    ${result}  View Private Label Assets    ${PrivateLabel_Asset_Id}
    run keyword if  '${result}' != '${PrivateLabel_Asset_Id}'     Fail   test1 Teardown
    log to console  Private Lable Asset ID: ${result}

26. Add Party To Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Add Party To PL  Private_Label/Add_Parties_To_Private_Label.json

27. Active Private Label(PL2)
    [Tags]	Functional	certificate	PL  POST    current
...
30. Expire base certificate
    [Tags]	Functional	certificate	create	POST    current
    set global variable  ${Certificate_Id}   ${Certificate_Id_Modify}
    Certify Certificate  Certificate/Certify_RegressionCertificatewithExpirydatepriortoTodaysdate_AfterModify.json
#    ${response}  Get Error Message  ${response_api}
#    log to console  ${response}
#    run keyword if  "${response}" != "Private label exists for base certificate. Expire the private label prior to the base certificate"   Fail	test1 Teardown

30. Validate Certificate Status
    [Tags]	Functional	certificate	create	current
    ${response}  Get certificate status
    log to console  ${response}
    run keyword if	${response} != "Obsolete"	Fail	test1 Teardown

31. Get Private Label certificate status
    [Tags]	Functional	PL	current
    ${response}  Get Private Label status   ${PrivateLabel_Id1}
    log to console  ${response}
    run keyword if	${response} != "Obsolete"	Fail	test1 Teardown

32. Get Private Label certificate status
    [Tags]	Functional	PL	current
    ${response}  Get Private Label status   ${PrivateLabel_Id}
    log to console  ${response}
    run keyword if	${response} != "Obsolete"	Fail	test1 Teardown