...
27. Certify Private Label (Withdraw)
    [Tags]	Functional	certificate	PL  POST    current
    Certify Private Label  Private_Label/Certify_PrivateLabelCertificatewithWDpriortoCD&EDempty.json
...
30. Validate Certificate Status
    ...
    run keyword if	${response1} != "Under Revision"	Fail	test1 Teardown

31. Withdraw base certificate
    [Tags]	Functional	certificate	create	POST    current
...
32. Validate Certificate Status
    [Tags]	Functional	certificate	create	current
    ${response1}  Get certificate status with certificateId   ${Certificate_Id_Modify}
    log to console  ${response1}
