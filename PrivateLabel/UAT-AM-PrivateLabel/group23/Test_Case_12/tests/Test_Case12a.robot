...
37. Modify Private Label1 for version3
    [Tags]	Functional	certificate	PL  POST    current
    Modify Private Lable for Version3    Private_Label/Modify_PrivateLabelCertificate_Revision3.json   ${PrivateLabel_Id_Modify}
...
43. Validate Private Label Status
    [Tags]	Functional	certificate PL	create	current
    ${response}  Get Private Label status   ${PrivateLabel_Id_Modify2}
    log to console  ${response}
    run keyword if	${response} != "Active"	Fail	test1 Teardown
    ${response1}  Get Private Label status   ${PrivateLabel2_Id_Modify2}
    log to console  ${response1}
    run keyword if	${response1} != "Active"	Fail	test1 Teardown
