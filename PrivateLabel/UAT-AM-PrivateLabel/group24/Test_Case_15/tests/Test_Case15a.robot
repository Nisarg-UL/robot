...
14. Edit Private Label Asset with New referenceNumber & referencenoChangeFlag - Y
    [Tags]	Functional	certificate	PL  create	POST    current
    Edit Private Label Asset taxonomy and model nomenclature  Edit_PL_Asset_NewReferenceNumber_ReferenceNoChangeFlag_Y.json   ${PrivateLabel_Id}
    should be empty  ${pl_asset_edit_error}
    ${success_models}   pl_asset_edit_values    ${pl_asset_edit_success}    ${modelName_key}
    should be equal  ${success_models}   ["PL_Regression_Test_Model_2", "${pl_model_name_1}_${current_time}"]
    ${success_pl_assetId}   pl_asset_edit_values    ${pl_asset_edit_success}    ${plAssetId_key}
    should be equal  ${success_pl_assetId}   ["${PrivateLabel_Asset_Id2}", "${PrivateLabel_Asset_Id1}"]
