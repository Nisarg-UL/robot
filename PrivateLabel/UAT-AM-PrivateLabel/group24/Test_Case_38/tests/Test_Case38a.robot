...
15. Edit Private Label Asset with Invalid PLAssetId
    [Tags]	Functional	certificate	PL  create	POST    current
    Edit Private Label Asset taxonomy and model nomenclature  Edit_PL_Asset_Invalid_plAssetId.json   ${PrivateLabel_Id}
    ${error_messages}  pl_asset_edit_values    ${pl_asset_edit_error}  ${message_key}
    should be equal  ${error_messages}   ["Invalid linkage between Asset and certificate"]
    ${error_pl_assetId}  pl_asset_edit_values    ${pl_asset_edit_error}  ${plAssetId_key}
    should be equal  ${error_pl_assetId}   ["ABCDEFGH-IJKL-MNOP-QRSTUVWXYZ00"]
    ...
    should be equal  ${success_models}   ["${pl_model_name_1}_${current_time}", "PL_Regression_Test_Model_2"]
    ${success_pl_assetId}  pl_asset_edit_values    ${pl_asset_edit_success}  ${plAssetId_key}
    should be equal  ${success_pl_assetId}   ["${PrivateLabel_Asset_Id1}", "${PrivateLabel_Asset_Id2}"]
