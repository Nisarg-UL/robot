...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group03/Test_Case_04/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_04/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group03/Test_Case_04/inputrequest/request3.json	${asset_Id_Product1}
...
19. Add Same Party To Private Label
    [Tags]	Functional	certificate	PL  create	POST    current
    Add Party To PL  Private_Label/Add_Parties_To_Private_Label_With_Cardinality_1.1.json