...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group03/Test_Case_02/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_02/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group03/Test_Case_02/inputrequest/request3.json	${asset_Id_Product1}
...
18. Add Party To Private Label
    [Tags]	Functional	certificate	PL  create	POST    current
    run keyword and ignore error  Add Party To PL  Private_Label/Add_Parties_Not_Present_In_Base_Certificate_To_Private_Label_Production_Site.json
    ${response}  get error message  ${response_api}
    run keyword if	"${response}" != "Production site not part of base certification and thus cannot be added to private label"	Fail	test1 Teardown
    log to console  ${response}