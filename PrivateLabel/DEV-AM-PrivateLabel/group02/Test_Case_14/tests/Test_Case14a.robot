...
10. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict  group02/Test_Case_14/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group02/Test_Case_14/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group02/Test_Case_14/inputrequest/request3.json	${asset_Id_Product1}
...
19. Add Asset To Private Label
    [Tags]	Functional	certificate	PL  create	POST    Notcurrent
    Add Asset To PL  Private_Label/Add_Asset_To_Private_Label_Change_Model.json
