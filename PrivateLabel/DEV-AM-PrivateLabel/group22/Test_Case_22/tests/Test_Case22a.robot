...
24. Modify Private Label by changing the certificate Type
    [Tags]	Functional	certificate	PL  Edit    POST    current
    ${response}     run keyword and ignore error    Modify Private Lable    Private_Label/Modify_PrivateLabelCertificate_changingCertificateType.json    ${PrivateLabel_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Exception occurred while validating the privateLabel - Asset Data Validation -> No data matches with list of values. Error while validating Certificate Type List of values Regression Scheme"  Fail	test1 Teardown
	log to console  ${msg}
