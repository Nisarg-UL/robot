...
13. Add Asset To Private Label
    [Tags]	Functional	certificate	PL  create	POST    current
    Add Asset To Private Label  Private_Label/Add_Asset_To_Private_Label.json   ${PrivateLabel_Id}

14. Edit Private Label Asset with Empty Taxonomy, Existing referenceNumber & Empty referencenoChangeFlag
    [Tags]	Functional	certificate	PL  create	POST    current
    run keyword and ignore error     Edit Private Label Asset taxonomy and model nomenclature  Edit_PL_Asset_EmptyTaxonomy_ExistingReferenceNumber_EmptyReferenceNoChangeFlag.json   ${PrivateLabel_Id}
    ${response}  get error message  ${response_api}
    run keyword if  "${response}" != "Invalid input passed in refNoChangeFlag key, Value can be Y/N"	Fail	test1 Teardown
