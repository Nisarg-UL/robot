...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_07/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group05/Test_Case_07/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group05/Test_Case_07/inputrequest/request3.json	${asset_Id_Product1}
...
22. Search for the Private Label Assets
    [Tags]	Functional	create  POST	current
    ${response}  Search Private label Asset    productType=Regression%20Test%20Product%201&ownerReference_PartySiteID=${asset_owner_ref}&referenceNumber=PL_DEMOFILE01&family_Series=Reg-Test&modelName=PL_Regression_Test_Model_1
