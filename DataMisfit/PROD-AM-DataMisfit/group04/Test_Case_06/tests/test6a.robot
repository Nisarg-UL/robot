...
12. Link Component to Product
    [Tags]	Functional	current
    ${result}   Link Components to Asset    group04/Test_Case_06/inputrequest/Link_Product1toComponent1a.json     ${asset_Id_Product1}
...
20. View Asset Component Details
    ...
    ${card_change_txt1}     Get Asset Linkage Value with seq    ${response}  cardinalityChange3_Text   1
	run keyword if  '${card_change_txt1}' != 'Portly'  Fail	test1 Teardown
	${card_change_txt2}     Get Asset Linkage Value with seq    ${response}  cardinalityChange3_Text   2
	run keyword if  '${card_change_txt2}' != 'Rotund'  Fail	test1 Teardown
	${card_change_txt3}     Get Asset Linkage Value with seq    ${response}  cardinalityChange3_Text   3
	run keyword if  '${card_change_txt3}' != 'Fat'  Fail	test1 Teardown
#    set test variable	${card_change_txt1}	${response[0]["hasAssetLinkage"][32]["value"]}
#	run keyword if  '${card_change_txt1}' != 'Portly'  Fail	test1 Teardown
#	set test variable	${card_change_txt2}	${response[0]["hasAssetLinkage"][33]["value"]}
#	run keyword if  '${card_change_txt2}' != 'Rotund'  Fail	test1 Teardown
#	set test variable	${card_change_txt3}	${response[0]["hasAssetLinkage"][34]["value"]}
#	run keyword if  '${card_change_txt3}' != 'Fat'  Fail	test1 Teardown
