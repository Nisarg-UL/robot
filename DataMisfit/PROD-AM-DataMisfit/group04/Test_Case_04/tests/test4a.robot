...
12. Link Component to Product
    [Tags]	Functional	current
    ${result}   Link Components to Asset    group04/Test_Case_04/inputrequest/Link_Product1toComponent1a.json     ${asset_Id_Product1}
...
20. View Asset Component Details
    ...
    ${card_change_int1}     Get Asset Linkage Value with seq    ${response}  cardinalityChange1_Integer   1
	run keyword if  '${card_change_int1}' != '1'  Fail	test1 Teardown
	${card_change_int2}     Get Asset Linkage Value with seq    ${response}  cardinalityChange1_Integer   2
	run keyword if  '${card_change_int2}' != '5'  Fail	test1 Teardown
	${card_change_int3}     Get Asset Linkage Value with seq    ${response}  cardinalityChange1_Integer   3
	run keyword if  '${card_change_int3}' != '9'  Fail	test1 Teardown
#    set test variable	${card_change_int1}	${response[0]["hasAssetLinkage"][30]["value"]}
#	run keyword if  '${card_change_int1}' != '1'  Fail	test1 Teardown
#	set test variable	${card_change_int2}	${response[0]["hasAssetLinkage"][31]["value"]}
#	run keyword if  '${card_change_int2}' != '5'  Fail	test1 Teardown
#	set test variable	${card_change_int3}	${response[0]["hasAssetLinkage"][32]["value"]}
#	run keyword if  '${card_change_int3}' != '9'  Fail	test1 Teardown
