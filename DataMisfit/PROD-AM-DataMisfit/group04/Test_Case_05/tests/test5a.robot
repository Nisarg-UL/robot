...
12. Link Component to Product
    [Tags]	Functional	current
    ${result}   Link Components to Asset    group04/Test_Case_05/inputrequest/Link_Product1toComponent1a.json     ${asset_Id_Product1}
...
16. End Date Misfit Testing PET_V3
	[Tags]	Functional	POST	current
	Expire PET   ${data_misfit_testing_v3_hierarchy_Id}

17. Post Date Misfit Testing PET_V4
	[Tags]	Functional	POST	current
...
20. View Asset Component Details
    ...
    ${card_change_dec1}     Get Asset Linkage Value with seq    ${response}  cardinalityChange2_Decimal   1
	run keyword if  '${card_change_dec1}' != '1.1'  Fail	test1 Teardown
	${card_change_dec2}     Get Asset Linkage Value with seq    ${response}  cardinalityChange2_Decimal   2
	run keyword if  '${card_change_dec2}' != '5.1'  Fail	test1 Teardown
	${card_change_dec3}     Get Asset Linkage Value with seq    ${response}  cardinalityChange2_Decimal   3
	run keyword if  '${card_change_dec3}' != '9.1'  Fail	test1 Teardown
#    set test variable	${card_change_dec1}	${response[0]["hasAssetLinkage"][31]["value"]}
#	run keyword if  '${card_change_dec1}' != '1.1'  Fail	test1 Teardown
#	set test variable	${card_change_dec2}	${response[0]["hasAssetLinkage"][32]["value"]}
#	run keyword if  '${card_change_dec2}' != '5.1'  Fail	test1 Teardown
#	set test variable	${card_change_dec2}	${response[0]["hasAssetLinkage"][33]["value"]}
#	run keyword if  '${card_change_dec2}' != '9.1'  Fail	test1 Teardown
