...
1b. End Date Misfit Testing PET_V3
	[Tags]	Functional	POST	current
	Expire PET   ${data_misfit_testing_v3_hierarchy_Id}

1c. End Date Misfit Testing PET_V4
	[Tags]	Functional	POST	current
...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_01/inputrequest/CreateProduct1.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    ${response}  run keyword and ignore error    Modify Product1 Asset   group01/Test_Case_01/inputrequest/ModifyProduct1.json    ${asset_Id_Product1}
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'Exception occurred while validating the product - Asset Data Validation -> No data matches with list of values. Error while validating Text Attribute 1 - LOV List of values Red; Green; Blue; Orange'  Fail	test1 Teardown
	log to console  ${msg}