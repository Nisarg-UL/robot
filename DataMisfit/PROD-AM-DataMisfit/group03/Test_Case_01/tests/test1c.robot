...
3. Asset Creation With POST Request
    ...
    create product1 asset	group03/Test_Case_01/inputrequest/CreateProduct1c.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    ${response}  run keyword and ignore error    Modify Product1 Asset   group03/Test_Case_01/inputrequest/ModifyProduct1c.json    ${asset_Id_Product1}
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'Exception occurred while validating the product - Asset Data Validation -> No data matches with list of values. Error while validating Text Attribute 6 - LOV List of values One; Two; Four; Five'  Fail	test1 Teardown
	log to console  ${msg}