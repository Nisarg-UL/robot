...
1. End Date Misfit Testing PET_V4
	[Tags]	Functional	POST	current
	Expire PET   ${data_misfit_testing_v4_hierarchy_Id}

2. Post Date Misfit Testing PET_V3
	[Tags]	Functional	POST	current
	Activate PET   ${data_misfit_testing_v3_hierarchy_Id}
...
10. End Date Misfit Testing PET_V3
	[Tags]	Functional	POST	current
	Expire PET   ${data_misfit_testing_v3_hierarchy_Id}

11. Post Date Misfit Testing PET_V4
	[Tags]	Functional	POST	current
	Activate PET   ${data_misfit_testing_v4_hierarchy_Id}
...
13. Modify Product1
    ...
    ${data_grp_att1}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute1   1
	run keyword if  '${data_grp_att1}' != '1'  Fail	test1 Teardown
	${data_grp_att2}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute2   1
	run keyword if  '${data_grp_att2}' != '0.1'  Fail	test1 Teardown
	${data_grp_att3}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute3   1
	run keyword if  '${data_grp_att3}' != 'Yes'  Fail	test1 Teardown
	${data_grp_att4}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute4   1
	run keyword if  '${data_grp_att4}' != 'Up'  Fail	test1 Teardown
	${data_grp_att12}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute1   2
	run keyword if  '${data_grp_att12}' != '2'  Fail	test1 Teardown
	${data_grp_att22}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute2   2
	run keyword if  '${data_grp_att22}' != '0.2'  Fail	test1 Teardown
	${data_grp_att32}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute3   2
	run keyword if  '${data_grp_att32}' != 'No'  Fail	test1 Teardown
	${data_grp_att42}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute4   2
	run keyword if  '${data_grp_att42}' != 'Down'  Fail	test1 Teardown
	${data_grp_att13}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute1   3
	run keyword if  '${data_grp_att13}' != '3'  Fail	test1 Teardown
	${data_grp_att23}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute2   3
	run keyword if  '${data_grp_att23}' != '0.3'  Fail	test1 Teardown
	${data_grp_att33}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute3   3
	run keyword if  '${data_grp_att33}' != ''  Fail	test1 Teardown
	${data_grp_att43}     Get Asset Attribute Value with seq    ${response_api.json()}  dataGroupAttribute4   3
