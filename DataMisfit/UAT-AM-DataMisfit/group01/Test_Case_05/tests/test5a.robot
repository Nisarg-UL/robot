...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_05/inputrequest/CreateProduct5.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    Modify Product1 Asset   group01/Test_Case_05/inputrequest/ModifyProduct5.json    ${asset_Id_Product1}
    ${data_grp_att1}     Get Asset Attribute Value    ${response_api.json()}  dataGroupAttribute1
	run keyword if  '${data_grp_att1}' != '3'  Fail	test1 Teardown
	${data_grp_att2}     Get Asset Attribute Value    ${response_api.json()}  dataGroupAttribute2
	run keyword if  '${data_grp_att2}' != '0.3'  Fail	test1 Teardown
	${data_grp_att3}     Get Asset Attribute Value    ${response_api.json()}  dataGroupAttribute3
	run keyword if  '${data_grp_att3}' != 'Yes'  Fail	test1 Teardown
	${data_grp_att4}     Get Asset Attribute Value    ${response_api.json()}  dataGroupAttribute4
