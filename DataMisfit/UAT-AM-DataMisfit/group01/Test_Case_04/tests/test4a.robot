...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_04/inputrequest/CreateProduct4.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    Modify Product1 Asset   group01/Test_Case_04/inputrequest/ModifyProduct4.json    ${asset_Id_Product1}
    set test variable	${txt_att4_uom}	${response_api.json()["data"]["attributes"][9]["uom"]}
	run keyword if  '${txt_att4_uom}' != 'second'  Fail	test1 Teardown
    ${txt_att4_value}     Get Asset Attribute Value   ${response_api.json()}  textAttribute4_LOV
	run keyword if  '${txt_att4_value}' != 'Beta'  Fail	test1 Teardown
