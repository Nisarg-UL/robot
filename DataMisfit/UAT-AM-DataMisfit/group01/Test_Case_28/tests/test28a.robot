...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_28/inputrequest/CreateProduct28.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    Modify Product1 Asset   group01/Test_Case_28/inputrequest/ModifyProduct28.json    ${asset_Id_Product1}
    ${card_change_txt1}     Get Asset Attribute Value with seq    ${response_api.json()}  cardinalityChange3_Text   1
	run keyword if  '${card_change_txt1}' != 'Portly'  Fail	test1 Teardown
	${card_change_txt2}     Get Asset Attribute Value with seq    ${response_api.json()}  cardinalityChange3_Text   2
	run keyword if  '${card_change_txt2}' != 'Rotund'  Fail	test1 Teardown
	${card_change_txt3}     Get Asset Attribute Value with seq    ${response_api.json()}  cardinalityChange3_Text   3
	run keyword if  '${card_change_txt3}' != 'Fat'  Fail	test1 Teardown
#    set test variable	${card_change_txt1}	${response_api.json()["data"]["attributes"][38]["value"]}
#	run keyword if  '${card_change_txt1}' != 'Portly'  Fail	test1 Teardown
#	set test variable	${card_change_txt2}	${response_api.json()["data"]["attributes"][39]["value"]}
#	run keyword if  '${card_change_txt2}' != 'Rotund'  Fail	test1 Teardown
#	set test variable	${card_change_txt3}	${response_api.json()["data"]["attributes"][40]["value"]}
#	run keyword if  '${card_change_txt3}' != 'Fat'  Fail	test1 Teardown
