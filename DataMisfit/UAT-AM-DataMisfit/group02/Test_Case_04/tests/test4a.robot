...
12. Link Component to Product
    [Tags]	Functional	current
    ${result}   Link Components to Asset    group02/Test_Case_04/inputrequest/Link_Product1toComponent1.json     ${asset_Id_Product1}
...
20. View Asset Component Details
    ...
    set test variable	${txt_att4_uom} 	${response[0]["hasAssetLinkage"][4]["uom"]}
	run keyword if  '${txt_att4_uom}' != 'first'  Fail	test1 Teardown
    ${txt_att4_value}     Get Asset Linkage Value   ${response}  textAttribute4_LOV
	run keyword if  '${txt_att4_value}' != 'Alpha'  Fail	test1 Teardown
