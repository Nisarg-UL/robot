...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_27/inputrequest/CreateProduct27.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    Modify Product1 Asset   group01/Test_Case_27/inputrequest/ModifyProduct27.json    ${asset_Id_Product1}
    ${card_change_dec1}     Get Asset Attribute Value with seq    ${response_api.json()}  cardinalityChange2_Decimal   1
	run keyword if  '${card_change_dec1}' != '1.1'  Fail	test1 Teardown
	${card_change_dec2}     Get Asset Attribute Value with seq    ${response_api.json()}  cardinalityChange2_Decimal   2
	run keyword if  '${card_change_dec2}' != '5.1'  Fail	test1 Teardown
	${card_change_dec3}     Get Asset Attribute Value with seq    ${response_api.json()}  cardinalityChange2_Decimal   3
	run keyword if  '${card_change_dec3}' != '9.1'  Fail	test1 Teardown
#    set test variable	${card_change_dec1}	${response_api.json()["data"]["attributes"][37]["value"]}
#	run keyword if  '${card_change_dec1}' != '1.1'  Fail	test1 Teardown
#	set test variable	${card_change_dec2}	${response_api.json()["data"]["attributes"][38]["value"]}
#	run keyword if  '${card_change_dec2}' != '5.1'  Fail	test1 Teardown
#	set test variable	${card_change_dec2}	${response_api.json()["data"]["attributes"][39]["value"]}
