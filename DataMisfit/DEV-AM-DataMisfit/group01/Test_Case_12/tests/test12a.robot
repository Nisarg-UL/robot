...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_12/inputrequest/CreateProduct12.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    ${response}  run keyword and ignore error    Modify Product1 Asset   group01/Test_Case_12/inputrequest/ModifyProduct12.json    ${asset_Id_Product1}
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'Exception occurred while validating the product - Error while validating Attribute-Changed Data Type 3 - T to B [Value Maybe ; Data Type- Boolean]'  Fail	test1 Teardown
	log to console  ${msg}
