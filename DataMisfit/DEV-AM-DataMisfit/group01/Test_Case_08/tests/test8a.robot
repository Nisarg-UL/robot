...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_08/inputrequest/CreateProduct8.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    Modify Product1 Asset   group01/Test_Case_08/inputrequest/ModifyProduct8.json    ${asset_Id_Product1}
    set test variable	${data_ItoDec}	${response_api.json()["data"]["attributes"][16]["value"]}
    ${data_ItoDec}     Get Asset Attribute Value   ${response_api.json()}  changedDataType1_ItoDec
	run keyword if  '${data_ItoDec}' != '1'  Fail	test1 Teardown
