...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_06/inputrequest/CreateProduct6a.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    ${response}  run keyword and ignore error   Modify Product1 Asset   group01/Test_Case_06/inputrequest/ModifyProduct6a.json    ${asset_Id_Product1}
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'Exception occurred while validating the product - Asset Data Validation -> Error while validating Integer Attribute 1 Range 0-10, Increment of Range 1'  Fail	test1 Teardown
	log to console  ${msg}
