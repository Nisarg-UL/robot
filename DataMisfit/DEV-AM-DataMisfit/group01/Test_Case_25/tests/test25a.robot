...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_25/inputrequest/CreateProduct25a.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    Modify Product1 Asset   group01/Test_Case_25/inputrequest/ModifyProduct25a.json    ${asset_Id_Product1}
    ${data_group_add1}     Get Asset Attribute Value   ${response_api.json()}  dataGroupAdd1
	run keyword if  '${data_group_add1}' != '10'  Fail	test1 Teardown
	${data_group_add2}     Get Asset Attribute Value   ${response_api.json()}  dataGroupAdd2
	run keyword if  '${data_group_add2}' != '9.9'  Fail	test1 Teardown
	${data_group_add3}     Get Asset Attribute Value   ${response_api.json()}  dataGroupAdd3
	run keyword if  '${data_group_add3}' != 'Yes'  Fail	test1 Teardown
	${data_group_add4}     Get Asset Attribute Value   ${response_api.json()}  dataGroupAdd4
