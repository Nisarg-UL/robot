...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_17/inputrequest/CreateProduct17b.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    ${response}  run keyword and ignore error    Modify Product1 Asset   group01/Test_Case_17/inputrequest/ModifyProduct17b.json    ${asset_Id_Product1}
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'Exception occurred while validating the product - Error while validating attribute- Changed Data Type 6a - T to Dec -Data Type- Decimal'  Fail	test1 Teardown
	log to console  ${msg}
