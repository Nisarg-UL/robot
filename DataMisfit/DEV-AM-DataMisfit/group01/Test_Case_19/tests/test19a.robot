...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_19/inputrequest/CreateProduct19.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    Modify Product1 Asset   group01/Test_Case_19/inputrequest/ModifyProduct19.json    ${asset_Id_Product1}

14. Get Details of an Asset for Content Type
	[Tags]	Functional	POST	current
    ${response}     Details of an Asset for Content Type     ${asset_Id_Product12}
    set test variable	${data_TtoC}	${response["data"]["attributes"][0]["value"]}
	run keyword if  '${data_TtoC}' != 'Thin Mints'  Fail	test1 Teardown
