...
3. Asset Creation With POST Request
    ...
    create product1 asset	group01/Test_Case_07/inputrequest/CreateProduct7a.json
...
13. Modify Product1
	[Tags]	Functional	POST	current
    ${response}  run keyword and ignore error    Modify Product1 Asset   group01/Test_Case_07/inputrequest/ModifyProduct7a.json    ${asset_Id_Product1}
    ${msg}  Get Error Message   ${response_api}
	run keyword if  '${msg}' != 'Exception occurred while validating the product - Asset Data Validation -> Error while validating Integer Attribute 2 Range 0-20, Increment of Range 5'  Fail	test1 Teardown
	log to console  ${msg}
