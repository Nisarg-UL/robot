...
10. Change Reference number
    [Tags]	Functional	POST	current
    run keyword and ignore error  Modify Taxonomy Single_Model With Asset_Id   change_refno_empty_taxonomy_chars_Single_Model.json
    ${message}   err msg  ${response_api}
    log to console  ${message}
    run keyword if  "${message}" != "reference number cannot be empty in request"  fail