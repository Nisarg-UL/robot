...
14. Change Family Series
    [Tags]	Functional	POST	current
    run keyword and ignore error  Modify Taxonomy Bulk_Model With Collection_Id   change_family_less_than_50_chars_Single_Model.json
    ${status}   err msg   ${response_api}
    run keyword if  "${status}" != "Cannot modify model number and series in collection level"  Fail