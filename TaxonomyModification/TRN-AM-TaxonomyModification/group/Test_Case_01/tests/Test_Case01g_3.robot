...
16. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group/Test_Case_01/inputrequest/request1.json	${asset_Id_Product12}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group/Test_Case_01/inputrequest/request2.json	${asset_Id_Product12}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group/Test_Case_01/inputrequest/request3.json	${asset_Id_Product12}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response3.text}
    run keyword if  '${clause}' != 'False'  Fail	test1 Teardown
...
#Customer requests to change Model number (ver1.0) for one of Immutable Asset in collection
#-Model number -  (< 50 charachters)
19. Change Model Number
    [Tags]	Functional	POST	current
    run keyword and ignore error  Modify Taxonomy Bulk_Model Without Collection_Id   change_model_less_than_150_chars_Bulk_Model.json
