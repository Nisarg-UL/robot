...
14. Change Reference number
    [Tags]	Functional	POST	current
    Modify Taxonomy Single_Model With Asset_Id   change_refno_less_than_50_chars_Single_Model.json
    ${status}   api_message   ${response_api}
    run keyword if  "${status}" != "Taxonomy updated successfully"  Fail

15. Change Reference number Again
    [Tags]	Functional	POST	current
    Modify Taxonomy Single_Model With Asset_Id   change_refno_less_than_50_chars_Single_Model_Multiple_Time.json
    ${status}   api_message   ${response_api}
    run keyword if  "${status}" != "Taxonomy updated successfully"  Fail