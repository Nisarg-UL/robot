...
*** Variables ***
${API_ENDPOINT}	http://10.40.76.70:8080/InformationPlatformServices/api/v${Api_ver}	#For UAT

@{database}	pymysql	infopltfrm_transDBv05	ul_transUser	q2Rbd6Wgpg	10.40.76.66 	3306	#DB for UAT
...
${certificate_hierarchy_Id}      ff152dbc-3ae5-46f7-8a15-678cfd9e5582          #for UAT
...
${standard_hierarchy_Id}          5c00f7dd-50cf-4dff-a0bf-cf0c9d1cc44b         #for UAT - with clause group
${standard_hierarchy_Id}          b6be9207-cd54-4ed5-9fee-7e3b4e5a1f30         #for UAT - no clause group
${Reg_prod1_metadataId}           4df920da-5aa5-491b-a424-78094f6aa8b2         #for UAT
${Reg_prod2_metadataId}           6a27d4bc-80c5-4c16-beda-ba38da2cfc35         #for UAT
${Reg_prod3_metadataId}           da456da5-daad-4eaa-be9e-b5090f1cae48         #for UAT
${noEvalReqd_hierarchy_Id}     ge28b891-a43e-4f59-9f15-2554f93359fe            #for UAT
${regression_product_1_hierarchy_id}    91ad0b15-ed4f-4056-b3f2-bd8031dc7bba   #for UAT
${regression_product_2_hierarchy_id}    0e552d19-3761-4a60-b27e-ac5df1d401c9   #for UAT
...
Get Asset From Endpoint
    ...
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/assetDetails/${assetId}?user=000000
...
Get Reviewer Summary Questions Details
   [Arguments]	${assetId}  ${assessmentCluaseId}
   ${response}=    urllib2.urlopen	${API_ENDPOINT}/assets/${assetId}/standards/evaluations/clause/${assessmentCluaseId}
...
Get Asset Details using UserId & Role
    ...
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/${assetId}?user=${user_id}&role=${role_name}
    ${json_convert}	json load data	${response}
    ${json}	convert	${json_convert}
    ${attr}     role_attributes     ${json}
    set global variable  ${Asset_attr}    ${attr}
...
Get Component Details using UserId
    [Arguments]	${assetId}   ${user_id}
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/${assetId}/component?user=${user_id}
...
Get Collection_ID
    [Arguments]	${assetId}
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/${assetId}?user=000000
    ${json_convert}	json load data	${response}
    ${json}	convert	${json_convert}
    ${result}	col_id	${json}
...
#with Older certficate API (Non-Modularized one)
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/certificateDetails?certificateId=${Certificate_Id}&user=000000
    ${json_convert}	json load data	${response}
    ${json}	convert	${json_convert}
...
Get Test Integer Attribute Value
    [Arguments]	${cert_Id}
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/certificate/${cert_Id}?user=000000
    ${json_convert}	json load data	${response}
    ${json}	convert	${json_convert}
...
Create Certificate2
   ...
   log to console	"Certificate Type": ${scheme}
   set test variable	${certificate_name}  ${result.json()["data"]["certificateName"]}
   set global variable	${Certificate_Name2}  ${certificate_name}
   log to console	"Certificate Name": ${certificate_name}
   set test variable	${owner_ref}	${result.json()["data"]["ownerReference"]}
...
Get certificate Details using Role
    ...
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/certificate/${cert_Id}?role=${role_name}
    set global variable  ${response_api}     ${response.read()}
#    ${json_convert}	json load data	${response_api}
#    ${json}	convert	${json_convert}
    ${attr}     cert_attributes     ${response_api}
    set global variable  ${cert_attr}    ${attr}
    ${status}	certificate_status	${response_api}
...
Get certificate Details using UserId & Role
    ...
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/certificate/${cert_Id}?user=${user_id}&role=${role_name}
    set global variable  ${response_api}     ${response.read()}
#    ${json_convert}	json load data	${response}
#    ${json}	convert	${json_convert}
    ${attr}     cert_attributes     ${response_api}
...
Get certificate Details using Mode
    ...
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/assets/certificate/${cert_Id}?mode=${mode}
    set global variable  ${response_api}     ${response.read()}
#    ${json_convert}	json load data	${response}
#    ${json}	convert	${json_convert}
    ${attr}     cert_attributes     ${response_api}
...
Get Test Integer Attribute Value from Private Label
    [Arguments]	${pl_Id}
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/privateLabels/${pl_Id}?user=000000
    ${json_convert}	json load data	${response}
    ${json}	convert	${json_convert}
    ${result}	test_integer	${json}
    clear cache
...
View Private Label Details using Mode
    ...
    ${response}=    urllib2.urlopen	${API_ENDPOINT}/privateLabels/${pl_Id}?mode=${mode}
    set global variable  ${response_api}  ${response.read()}
#    ${json_convert}	json loads data	${response_search_api}
#    ${json}	convert	${json_convert}
    ${attr}     cert_attributes     ${response_api}
...
Configure Role Access
   ...
   ${result}=	Post Data To Endpoint	/hierarchy/${hierarchy_type}/accessRoles    ${File}	200
...
Get Role Access at Attribute Level
    ...
    ${response}=    urllib2.urlopen	${API_ENDPOINT}/hierarchy/${hierarchy_type}/attributes/accessRoles?entityName=${entity_name}&tabName=${tab_name}&user=${user}
    set global variable  ${response_api}  ${response.read()}
...
Validate Certificate Reference Attributes
    [Arguments]	${cert_Id}
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/certificate/${cert_Id}/referencedata/validate
    set global variable  ${response_api}     ${response.read()}
    ${json_convert}	json loads data	${response_api}
    ${json}	convert	${json_convert}
...
Get Associated Attributes of a Product
    ...
    ${response}=	urllib2.urlopen	${API_ENDPOINT}/certificate/${cert_Id}/referencedata/associatedAttributes?assetId=${assetId}
    set test variable  ${response_api}     ${response.read()}
    ${json_convert}	json loads data	${response_api}
    ${json}	convert	${json_convert}
#    set global variable  ${response_api}    ${json}
