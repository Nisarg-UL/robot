...
12. Certify certificate
    [Tags]	Functional	certificate	create	POST    current
    run keyword and ignore error  Add Decisions to Certificate using skipValidation  Certificate/Certify_RegressionCertificate.json     12345  false
    ${error_msg}  Get Error Message   ${response_api}
    run keyword if  "${error_msg}" != "Value doesn't exist in DB for this certificateId"     Fail
