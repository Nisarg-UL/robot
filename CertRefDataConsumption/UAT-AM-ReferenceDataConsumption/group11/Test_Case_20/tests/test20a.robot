...
21. Modify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    run keyword and ignore error  Modify Private Lable    Private_Label/Modify_PrivateLabelCertificate_withReferenceAttributes.json   12345
    ${error_msg}  Get Error Message   ${response_api}
    run keyword if  "${error_msg}" != "Value doesn\'t exist in DB for this private Label Id"     Fail
