...
11. Edit certificate
    [Tags]	Functional	certificate create	POST    current
    run keyword and ignore error  edit certificate   Certificate/CreationOfRegressionSchemeCertificate_withEmptyReferenceAttributes.json    ${EMPTY}
    ${error_msg}  Get Error Message   ${response_api}
    run keyword if  "${error_msg}" != "Cannot create as a certificate with this name and owner reference already exists under this scheme"     Fail
