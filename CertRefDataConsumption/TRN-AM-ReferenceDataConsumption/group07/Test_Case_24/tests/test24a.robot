...
8. Certificate Creation
	...
	set test variable  ${Regression_schemeScopeId}   123456
...
12. Certify certificate
    [Tags]	Functional	certificate	create	POST    current
    run keyword and ignore error  Add Decisions to Certificate using skipValidation  Certificate/Certify_RegressionCertificate.json     ${Certificate_Id}  false
    ${error_msg}  Get Error Message   ${response_api}
    run keyword if  '${error_msg}' != 'Exception occurred while validating the certificate - Mandatory params certificationProductType are missing in input JSON of Certificate'     Fail
...
#	should be empty  ${Ref_attr_names}
    run keyword if  '${Ref_attr_names}' != 'Standard'     Fail
    ${pk1}   Get Primary Keys from ref_attributes Table   ${Certificate_Id}  1
    run keyword if  '${pk1}' != '${Validation_standard_Id}'     Fail
    ${pk2}   Get Primary Keys from ref_attributes Table   ${Certificate_Id}  2
    run keyword if  '${pk2}' != '${Validation_standardLabel_Id}'     Fail
