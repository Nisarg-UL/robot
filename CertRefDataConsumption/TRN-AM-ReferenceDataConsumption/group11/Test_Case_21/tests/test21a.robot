...
21. Modify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    run keyword and ignore error  Modify Private Lable    Private_Label/Modify_PrivateLabelCertificate_withReferenceAttributes.json   ${EMPTY}
    ${error_msg}  Get Error Message   ${response_api}
    run keyword if  "${error_msg}" != "Private label already exists with same name for the given certificateType and ownerReference."     Fail

22. Disfigure Role With POST Request
	[Tags]	Functional	POST    current
