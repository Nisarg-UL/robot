...
12. Search Certificate with ownerReferenceList
...
12a. Validate Certificate Details
	...
	Compare lists  [${certificate_status}, ${certificate_version}, ${revision_number}, ${certify}]   [["${status_Under_Revision}", "${status_Active}"], ["${value_as_1.0}", "${value_as_1.0}"], ["${value_as_0}", "${value_as_0}"], ["${value_as_N}", "${value_as_Y}"]]
	...
	compare lists  [${CS_issueDate}, ${CS_revisionDate}, ${CS_withdrawalDate}, ${CS_expiryDate}]     [["${EMPTY}", "${today_date} ${time_00}"], ["${EMPTY}", "${future_date_1} ${time_00}"], ["${EMPTY}", "${future_date_2} ${time_00}"], ["${EMPTY}", "${future_date_3} ${time_00}"]]
