...
9. Search Certificate with all searchParameters
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${operator_value}   ${exact_search_value}
	Paginated Search for Certificate    Certificate_Search_with_all_searchParameters.json
...
9b. Validate findKeys searchParameters Details
    [Tags]	Functional	asset   Search	POST    current
    ...
    compare lists  ${SP_issuingBody_values}  ["${issuing_body_1}", "${exact_search_value}"]
    Extract mark values from searchParameters dictionary   ${FK_searchParameters_dict}
    compare lists  ${SP_mark_values}  ["${mark_1}", "${exact_search_value}"]
    Extract Certificate ownerReference values from searchParameters dictionary   ${FK_searchParameters_dict}
    compare lists  ${SP_cert_ownerReference_values}  ["${Cert_Owner_Ref}", "${exact_search_value}"]
