...
6. Certificates creation
    ...
    set test variable  ${mark_1}   ${mark_2}
...
11. Search Certificate with multiple markTypeList values
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${list_name}   ${markList_key}
	set global variable  ${list_key}   ${mark_key}
	set global variable  ${list_value1}   ${mark_1}
	set global variable  ${list_value2}   ${mark_2}
...
11a. Validate Certificate Details
	...
	compare lists  [${CS_certificate_type}, ${CS_certificate_name}, ${CS_Cerificate_owner_reference}, ${CS_issuing_body}, ${CS_mark}, ${CS_cert_ccn}]    [["${certificate_type_1}", "${certificate_type_1}"], ["${certificate_name}", "${cert_name2}"], ["${Cert_Owner_Ref}", "${Cert_Owner_Ref}"], ["${issuing_body_1}", "${issuing_body_1}"], ["${mark_1}", "${mark_2}"], ["${Scope_Code_1}", "${Scope_Code_1}"]]
...
11b. Validate findKeys Lists Details
    ...
    compare lists  ${FK_mark_values}  ["${mark_1}", "${mark_2}"]