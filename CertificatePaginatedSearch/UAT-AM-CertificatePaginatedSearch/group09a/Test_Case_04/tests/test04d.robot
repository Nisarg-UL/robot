...
9. Certify certificates
    ...
    set test variable  ${today_date}  ${yesterday_date}
...
11. Search Certificate with orderBy as '-issueDate'
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${orderBy_value}    -${issueDate_key}
...
11a. Validate Certificate Details
	...
	compare lists  [${CS_issueDate}, ${CS_revisionDate}, ${CS_withdrawalDate}, ${CS_expiryDate}]     [["${today_date} ${time_00}", "${yesterday_date} ${time_00}"], ["${future_date_1} ${time_00}", "${future_date_1} ${time_00}"], ["${future_date_2} ${time_00}", "${future_date_2} ${time_00}"], ["${future_date_3} ${time_00}", "${future_date_3} ${time_00}"]]
