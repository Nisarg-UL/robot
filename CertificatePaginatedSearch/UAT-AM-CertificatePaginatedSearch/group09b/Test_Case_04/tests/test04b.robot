...
12. Search Certificate with fromModifiedDate as current date & toModifiedDate as future date
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${fromDate_value}   ${today_date}${time_min_TZ}
	set global variable  ${fromDate_operator}   ${greater_than_or_equal}
	set global variable  ${toDate_value}     ${future_date_3}${time_max_TZ}
	set global variable  ${toDate_operator}    ${less_than_or_equal}
	Paginated Search for Certificate    Certificate_Search_with_modifiedDates.json
