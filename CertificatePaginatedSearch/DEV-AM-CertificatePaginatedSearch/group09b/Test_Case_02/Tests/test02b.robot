...
9. Certify certificate
    [Tags]	Functional	certificate	create	POST    current
    set test variable  ${future_date_3}  ${today_date}
...
11. Validate Certificate Status
    ...
    should be equal  ${cert_status}  "${status_Obsolete}"
    set test variable  ${Certificate_Id}    ${Certificate_Id2}
    ${cert_status}  Get certificate status
    should be equal  ${cert_status}  "${status_Under_Revision}"

12. Search Certificate with 'fromExpiryDate' as current date & 'toExpiryDate' as future date
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${fromExpiryDate_value}  ${today_date}
	set global variable   ${toExpiryDate_value}   ${future_date_3}
	Paginated Search for Certificate    Certificate_Search_with_expiryDates.json
...
12a. Validate Certificate Details
	...
	Compare lists  [${certificate_status}, ${certificate_version}, ${revision_number}, ${certify}]   [["${status_Obsolete}"], ["${value_as_1.0}"], ["${value_as_0}"], ["${value_as_Y}"]]
	...
	compare lists  [${CS_issueDate}, ${CS_revisionDate}, ${CS_withdrawalDate}, ${CS_expiryDate}]     [["${today_date} ${time_00}"], ["${future_date_1} ${time_00}"], ["${future_date_2} ${time_00}"], ["${today_date} ${time_00}"]]
