...
9. Validate Certificates Status
...
10. Search Certificate with multiple partiesRelationshipTypeList values
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${list_name}   ${partiesRelationshipTypeList_key}
	set global variable  ${list_key}   ${partiesRelationshipType_key}
	set global variable  ${list_value1}   ${party_brand_owner}
	set global variable  ${list_value2}   ${party_local_representative}
...
10a. Validate Certificate Details
	...
	compare lists  [${CS_certificate_type}, ${CS_certificate_name}, ${CS_Cerificate_owner_reference}, ${CS_issuing_body}, ${CS_mark}, ${CS_cert_ccn}]    [["${certificate_type_1}", "${certificate_type_1}"], ["${cert_name2}", "${certificate_name}"], ["${Cert_Owner_Ref}", "${Cert_Owner_Ref}"], ["${issuing_body_1}", "${issuing_body_1}"], ["${mark_1}", "${mark_1}"], ["${Scope_Code_1}", "${Scope_Code_1}"]]
...
10b. Validate findKeys Lists Details
    [Tags]	Functional	asset   Search	POST    current
    Extract Certificate partiesRelationshipTypeList values from findKeys dictionary  ${certificate_findkeys}
    compare lists  ${FK_partiesRelationshipType_values}  ["${party_brand_owner}", "${party_local_representative}"]