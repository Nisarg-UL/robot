...
11. Search Certificate with multiple certificateTypeList values
...
11a. Validate Certificate Details
	...
	compare lists  [${unique_certificateId}, ${CS_certificate_Id}, ${CS_certificate_hierarchyId}, ${CS_partySiteContainerId}]     [["${Certificate_Id}", "${Certificate_Id2}"], ["${Certificate_Id}", "${Certificate_Id2}"], ["${certificate_hierarchy_Id}", "${certificate2_hierarchy_Id}"], ["${EMPTY}", "${EMPTY}"]]
	compare lists  [${CS_certificate_type}, ${CS_certificate_name}, ${CS_Cerificate_owner_reference}, ${CS_issuing_body}, ${CS_mark}, ${CS_cert_ccn}]    [["${certificate_type_1}", "${certificate_type_2}"], ["${certificate_name}", "${cert_name2}"], ["${Cert_Owner_Ref}", "${Cert_Owner_Ref}"], ["${issuing_body_1}", "${issuing_body_1}"], ["${mark_1}", "${mark_1}"], ["${Scope_Code_1}", "${Scope_Code_2}"]]
