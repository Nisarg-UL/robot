...
9. Certify certificates
    ...
    set test variable  ${future_date_2}  ${tomorrow_date}
...
11. Search Certificate with orderBy as '-withdrawalDate'
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${orderBy_value}   -${withdrawalDate_key}
...
11a. Validate Certificate Details
	...
	compare lists  [${CS_issueDate}, ${CS_revisionDate}, ${CS_withdrawalDate}, ${CS_expiryDate}]     [["${today_date} ${time_00}", "${today_date} ${time_00}"], ["${future_date_1} ${time_00}", "${future_date_1} ${time_00}"], ["${future_date_2} ${time_00}", "${tomorrow_date} ${time_00}"], ["${future_date_3} ${time_00}", "${future_date_3} ${time_00}"]]
