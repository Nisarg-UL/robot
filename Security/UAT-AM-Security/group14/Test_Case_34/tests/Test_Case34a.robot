...
12. Get Questions from Certificate
	[Tags]	Functional	certificate create	POST    current
    ${has_questions}   Get Questions from Certificate using UserId & Role    ${Certificate_Id}    ${user}  Public;Owner;Brand%20Owner;Production%20Site;Local%20Representative;Applicant
    Should not be Empty     ${has_questions}
    ${questions}	get_questions  ${cert_questions}
    run keyword if  ${questions} != ['Question1', 'Question2', 'Question3', 'Question4', 'Question5', 'Question6', 'Question7', 'Question8', 'Question9']     Fail
