...
12. Get Questions from Certificate
	[Tags]	Functional	certificate create	POST    current
    ${has_questions}   Get Questions from Certificate using UserId & Role    ${Certificate_Id}    ${EMPTY}  Public;Owner;Brand%20Owner;Production%20Site;Local%20Representative;Applicant
    Should not be Empty     ${has_questions}
    ${questions}	get_questions  ${cert_questions}
    ${list}  convert_str_to_list     ['Question9', 'Question8', 'Question7', 'Question6', 'Question5']
    ${compare}   compare list of items  ${questions}  ${list}
