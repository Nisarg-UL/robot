...
14. View Private Label details using UserId & Role
	[Tags]	Functional	certificate create	POST    current
	View Private Label details using UserId & Role     ${PrivateLabel_Id}   ${EMPTY}   Local%20Representative
    should not be empty   ${cert_attr}
    ${attr_name}	get_attribute_names  ${cert_attr}   name
    run keyword if  ${attr_name} != ['IR Reference', 'TGA Reference', 'Footnote Symbol', 'Footnote Text']     Fail
