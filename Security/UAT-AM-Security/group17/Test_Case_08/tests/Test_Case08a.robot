...
12. Get Recommendation from Certificate
	[Tags]	Functional	certificate create	POST    current
    ${has_recommendation}   Get Recommendation from Certificate using Role    ${Certificate_Id}    Public;Owner;Brand%20Owner;Production%20Site;Local%20Representative;Applicant
    Should not be Empty     ${has_recommendation}
    ${recommendation}	get_questions  ${cert_recommendation}
    ${list}  convert_str_to_list     ['Review Date', 'Reviewer Employee Number', 'Reviewer Employee Name', 'Recommendation Comment', 'Recommendation Status']
