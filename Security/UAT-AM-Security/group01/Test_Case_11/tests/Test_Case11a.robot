...
2. Role Access Configuration With POST Request
	[Tags]	Functional	POST    current
    run keyword and ignore error     Configure Role Access    Certificate/ConfigureRole_LocalRepresentative_sameAttrAsPublic_forRegressionScheme.json   Certificate
...
4. Disfigure Role With POST Request
	[Tags]	Functional	POST    current
    Disfigure Role Access    Certificate/DisfigureRole_LocalRepresentative_sameAttrAsPublic_forRegressionScheme.json   Certificate
