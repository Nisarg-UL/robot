...
11. Get Parties from Certificate
	[Tags]	Functional	certificate create	POST    current
    Get Parties from Certificate using UserId & Role    ${Certificate_Id}    ManojAutomation   Public;Owner;Brand%20Owner;Production%20Site;Local%20Representative;Applicant
    Should not be Empty     ${cert_parties}
    ${parties}	get_parties  ${cert_parties}   relationshipType
    run keyword if  ${parties} != ['Brand Owner', 'Production Site', 'Applicant', 'Local Representative', 'Owner Reference']     Fail
