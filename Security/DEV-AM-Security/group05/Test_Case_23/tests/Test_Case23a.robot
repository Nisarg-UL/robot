...
14. View Private Label details using UserId & Role
	[Tags]	Functional	certificate create	POST    current
	View Private Label details using UserId & Role     ${PrivateLabel_Id}   ${EMPTY}   Applicant
    should not be empty   ${cert_attr}
    ${attr_name}	get_attribute_names  ${cert_attr}   name
    run keyword if  ${attr_name} != ['Test Date', 'Electrical Efficiency Rated', 'Energy Efficiency Rating']     Fail
