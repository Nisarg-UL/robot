...
12. Get Recommendation from Certificate
	[Tags]	Functional	certificate create	POST    current
    ${has_recommendation}   Get Recommendation from Certificate using UserId & Role    ${Certificate_Id}    ${user}  Public;Owner;Brand%20Owner;Production%20Site;Local%20Representative;Applicant
    Should not be Empty     ${has_recommendation}
    ${recommendation}	get_questions  ${cert_recommendation}
    run keyword if  ${recommendation} != ['Recommendation Status', 'Recommendation Comment', 'Reviewer Employee Name', 'Reviewer Employee Number', 'Review Date']    Fail
