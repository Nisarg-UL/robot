...
14. View Private Label Asset details using UserId
	[Tags]	Functional	certificate create	POST    current
	${result}    View Private Label Assets using Role     ${PrivateLabel_Asset_Id}   ${user_id}
    run keyword if  "${result}" != "${PrivateLabel_Asset_Id}"     Fail

15. Get Asset details using UserId
	[Tags]	Functional	certificate create	POST    current
	Get Asset Details using UserId     ${asset_Id_Product1}   ${user_1}
