...
15. View Private Label Assets using UserId & Role
	[Tags]	Functional	certificate create	POST    current
	${result}    View Private Label Assets using UserId & Role     ${PrivateLabel_Asset_Id}   ${EMPTY}   Applicant
    run keyword if  "${result}" != "${PrivateLabel_Asset_Id}"     Fail

16. Get Asset Details using UserId & Role
	[Tags]	Functional	certificate create	POST    current
	Get Asset Details using UserId & Role     ${asset_Id_Product1}   ${EMPTY}   Applicant
