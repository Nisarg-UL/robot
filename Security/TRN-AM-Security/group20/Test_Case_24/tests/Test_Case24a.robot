...
12. Get Certify from Certificate
	[Tags]	Functional	certificate create	POST    current
    ${has_certify}   Get Certify from Certificate using UserId & Role    ${Certificate_Id}    ${EMPTY}  Public;Owner;Brand%20Owner;Production%20Site;Local%20Representative;Applicant
    Should not be Empty     ${has_certify}
    ${certify}	get_questions  ${cert_certify}
    ${list}  convert_str_to_list     ['Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']
    ${compare}   compare list of items  ${certify}  ${list}
