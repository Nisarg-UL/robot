...
14. View Private Label details using UserId & Role
	[Tags]	Functional	certificate create	POST    current
	View Private Label details using UserId & Role     ${PrivateLabel_Id}   ${EMPTY}   Brand%20Owner
    should not be empty   ${cert_attr}
    ${attr_name}	get_attribute_names  ${cert_attr}   name
    run keyword if  ${attr_name} != ['Model Numbers', 'Certification Scheme Product Type', 'Certification Scheme Product Type Code']     Fail
