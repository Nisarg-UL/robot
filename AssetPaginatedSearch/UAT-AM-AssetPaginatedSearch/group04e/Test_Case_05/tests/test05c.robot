...
7. Search Asset with Search Text & family_Series Refiner
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${searchText_value}    ${Asset_Owner_Ref}
	set global variable  ${field_value}  ${family_Series_key}
...
7a. Validate Asset Details
	...
	Compare lists  [${asset_version}, ${isPLAsset}, ${Asset_created_by}]   [["${value_as_1.0}", "${value_as_1.0}"], ["${value_as_N}", "${value_as_N}"], ["${user_2}", "${user_2}"]]
	compare lists  ${Asset_model_nomenclature}   ["${modelNomenclature_1}", "${EMPTY}"]
	...
	compare lists  ${Asset_family_series}    ["${family_series_1}-${current_time}", "${family_series_2}-${current_time}"]
...
7b. Validate Refiner Details
	...
	compare lists  ${RF_family_Series_list}   ['${family_series_1}-${current_time}', ${value_as_1}, '${family_series_2}-${current_time}', ${value_as_1}]
