...
6. Complete Evaluation
	[Tags]	Functional	POST	current
	Complete Evaluation	markevaluationcomplete_InActive.json	${asset_Id_Product1}
...
8. Search Asset with Completed as Empty & Accepted as 'N'
	[Tags]	Functional	asset   Search	POST    current
	set global variable  ${completed_value}  ${EMPTY}
	set global variable  ${accepted_value}   ${value_as_N}
...
8a. Validate Asset Details
	...
	Compare lists  [${Asset_status}, ${asset_version}, ${isPLAsset}, ${Asset_model_nomenclature}, ${Asset_created_by}, ${Asset_updated_by}]   [["${status_InActive}"], ["${value_as_1.0}"], ["${value_as_N}"], ["${modelNomenclature_1}"], ["${user_2}"], ["${user_1}"]]
