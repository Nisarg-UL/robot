...
8. Search Asset with valid modelName values
	[Tags]	Functional	asset   Search	POST    current
	set test variable  ${like_search_value}  ${EMPTY}
	Search Asset    Asset_Search_with_modelName_likeSearch.json
...
8b. Validate findKeys Details
    ...
    compare lists   ${SP_modelName_values}    ["${today_date}", "${like_search_value}"]