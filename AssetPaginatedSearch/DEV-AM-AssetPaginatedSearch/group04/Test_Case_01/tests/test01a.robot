...
#	set global variable  ${completed_value}  ${value_as_N}
...
2d. Validate findKeys Lists Details
    [Tags]	Functional	asset   Search	POST    current
	...
	Extract ownerReference_PartySiteIDList values from findKeys dictionary  ${Asset_findkeys}
    compare lists  ${FK_ownerReference_PartySiteID_values}  ["${Asset_Owner_Ref}"]

2e. Validate searchParameters Details
    [Tags]	Functional	asset   Search	POST    current
