...
17. Certificate Audit with Certificate Params
    [Tags]	Functional	current
    ${response}  Certificate Audit  certificateType=Regression%20Scheme&ownerReference=${Cert_Owner_Ref}&certificateName=Regression-US001-1
    ${revision_no}    Get Revision Number   ${response}
	run keyword if  ${revision_no} != 0     Fail	test1 Teardown
