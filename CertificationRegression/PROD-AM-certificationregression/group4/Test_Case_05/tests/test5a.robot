...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group4/Test_Case_05/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group4/Test_Case_05/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group4/Test_Case_05/inputrequest/request3.json	${asset_Id_Product1}
...
15. Certify certificate with Recommendation Status as None
    [Tags]	Functional	certificate	create	POST    current
    Certify Certificate  Certificate/Certify_RegressionCertificate_WithRecommendationStatusAsNone.json
...
16. Validate Certificate Recommendation Status
    ...
    run keyword if	${response} != "None"	Fail	test1 Teardown
