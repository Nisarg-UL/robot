...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group3/Test_Case_01/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group3/Test_Case_01/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group3/Test_Case_01/inputrequest/request3.json	${asset_Id_Product1}
...
13. Lock Certificate with Certificate Parameters
    [Tags]	Functional	current
    ${response}  Lock Certificate  Lock_Template.json    certificateType=Regression%20Scheme&ownerReference=${Cert_Owner_Ref}&certificateName=Regression-US001-1
    ${msg}  Get Certificate Lock/Unlock Message   ${response_api}
	run keyword if  '${msg}' != 'certificate locked'  Fail	test1 Teardown
	log to console  ${msg}

13. UnLock Certificate with missing Certificate Parameters
    [Tags]	Functional	current
    ${response}  run keyword and ignore error  Unlock Certificate with Certificate Parameters  Unlock_Template.json    certificateType=Regression%20Scheme&certificateName=Regression-US001-1
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "'ownerReference' mandetory key missing in query parameter"  Fail	test1 Teardown
	log to console  ${msg}
