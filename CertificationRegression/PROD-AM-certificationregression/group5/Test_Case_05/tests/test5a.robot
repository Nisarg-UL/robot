...
14. Unlock Certificate with Certificate parameters
    [Tags]	Functional	current
    ${response}  Unlock Certificate with Certificate Parameters  Unlock_Template.json    certificateType=Regression%20Scheme&ownerReference=${Cert_Owner_Ref}&certificateName=Regression-US001-1
    ${msg}  Get Certificate Lock/Unlock Message   ${response_api}
	run keyword if  '${msg}' != 'certificate unlocked'  Fail	test1 Teardown
	log to console  ${msg}