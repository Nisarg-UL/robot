...
13. Lock Certificate with CertificateID
    [Tags]	Functional	current
    ${response}  Lock Certificate  Lock_Template.json    certificateId=${Certificate_Id}
    ${msg}  Get Certificate Lock/Unlock Message   ${response_api}
	run keyword if  '${msg}' != 'certificate locked'  Fail	test1 Teardown
	log to console  ${msg}

14. Unlock Certificate with CertificateID
    [Tags]	Functional	current
    ${response}  Unlock Certificate with Certificate Parameters  Unlock_Template.json    certificateId=${Certificate_Id}
    ${msg}  Get Certificate Lock/Unlock Message   ${response_api}
