...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group1/Test_Case_02/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group1/Test_Case_02/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group1/Test_Case_02/inputrequest/request3.json	${asset_Id_Product1}
...
13. Certification Mark
    ...
    run keyword if  ${Result} != "Listed"      Fail    test1 Teardown

14. Edit certificate
    [Tags]	Functional	certificate create	POST    current
    create certificate  Certificate/EditOfRegressionSchemeCertificate.json
