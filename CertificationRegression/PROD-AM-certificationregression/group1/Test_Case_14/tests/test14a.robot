...
12. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group1/Test_Case_14/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group1/Test_Case_14/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group1/Test_Case_14/inputrequest/request3.json	${asset_Id_Product1}
...
17. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group1/Test_Case_14/inputrequest/request1.json	${asset_Id_Product12}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group1/Test_Case_14/inputrequest/request2.json	${asset_Id_Product12}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group1/Test_Case_14/inputrequest/request3.json	${asset_Id_Product12}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response3.text}
...
20. Unlink Product and Evaluation to Certificate
    ...
    ${response}  Get ULAssetID   ${asset_Id_Product1}
    set global variable	${ul_asset_Id}  ${response}
    Link Product to Certificate  Unlink_Product_&_Evaluation.json