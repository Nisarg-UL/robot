...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group3/Test_Case_43/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group3/Test_Case_43/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group3/Test_Case_43/inputrequest/request3.json	${asset_Id_Product1}
...
14. Search Certificate with all Certificate Type and Issuing Body and Mark and Certificate Name
    [Tags]	Functional	current
    ${response}  Search Certificate   certificateType=Regression%20Scheme&issuingBody=USA&mark=Listed&certificateName=Regression-US001-1
