...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group2/Test_Case_15/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group2/Test_Case_15/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group2/Test_Case_15/inputrequest/request3.json	${asset_Id_Product1}
...
14. Search Certificate with Issuing body and Certificate Name
    [Tags]	Functional	current
    ${response}  Search Certificate   issuingBody=USA&certificateName=Regression-US001-1
