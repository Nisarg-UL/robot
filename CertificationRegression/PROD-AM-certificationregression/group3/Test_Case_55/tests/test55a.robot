...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group3/Test_Case_55/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group3/Test_Case_55/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group3/Test_Case_55/inputrequest/request3.json	${asset_Id_Product1}
...
14. Search Certificate with Mark and Product Type and Owner Reference and Certificate Name
    [Tags]	Functional	current
    ${response}  Search Certificate   mark=Listed&productType=Regression+Test+Product+1&ownerReference=${Cert_Owner_Ref}&certificateName=Regression-US001-1
