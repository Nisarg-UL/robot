...
*** Keywords ***
test1 Teardown
	Log To Console	test Teardown Beginning
	Expire The Asset	${asset_Id_Product1}
	Log To Console	test Teardown Finished
...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group2/Test_Case_05/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group2/Test_Case_05/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group2/Test_Case_05/inputrequest/request3.json	${asset_Id_Product1}
...
13. Link Product and Evaluation to Certificate
    ...
    Link Product to Certificate  Certificate/Link_Product1_Eval1_Regression2Certificate.json

14. Associate parties to certificate
    [Tags]	Functional	certificate	create	POST    current
    Get HasAssets   Regression2%20Scheme   Regression2-US001-1
    Get HasEvaluations  Regression2%20Scheme   Regression2-US001-1
    associate parties to certificate    Certificate/Associate_Parties_Regression2Certificate.json