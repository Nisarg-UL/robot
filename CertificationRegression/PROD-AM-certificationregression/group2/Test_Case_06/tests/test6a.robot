...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group2/Test_Case_06/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group2/Test_Case_06/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group2/Test_Case_06/inputrequest/request3.json	${asset_Id_Product1}
...
15. Associate New parties to certificate
    [Tags]	Functional	certificate	create	POST    current
    Get HasAssets   Regression%20Scheme   Regression-US001-1
    Get HasEvaluations  Regression%20Scheme   Regression-US001-1
    run keyword and ignore error  associate parties to certificate    Certificate/Associate_New_Brand_Name_Owner_Parties_Certificate.json