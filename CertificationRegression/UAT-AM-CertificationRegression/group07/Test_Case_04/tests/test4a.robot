...
13. Edit certificate using Invalid Certificate Id
    [Tags]	Functional	certificate create	POST    current
    ${response}     run keyword and ignore error    edit certificate  Certificate/EditOfRegressionSchemeCertificate.json    123456
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Value doesn't exist in DB for this certificateId"  Fail	test1 Teardown
	log to console  ${msg}
