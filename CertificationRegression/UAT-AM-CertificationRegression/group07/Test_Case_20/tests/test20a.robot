...
18. Post Date CET_V2.2
	[Tags]	Functional	POST	current
	Activate CET   ${certificate_hierarchy_IdV2.2}

19. Modify Certificate
	[Tags]	Functional	certificate create	POST    current
    ${response}     run keyword and ignore error    Modify Certificate    Certificate/Modify_RegressionSchemeCertificate.json   ${Certificate_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "The following party role(s) - Local Representative are invalid per the latest Certificate template and have been removed automatically. Associated Private Label Certificates need to be modified accordingly."  Fail	test1 Teardown
	log to console  ${msg}

20. End Date CET_V2.2
	[Tags]	Functional	POST	current
