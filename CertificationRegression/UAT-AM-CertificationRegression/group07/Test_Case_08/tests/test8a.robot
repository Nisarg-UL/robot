...
17. Modify Certificate without changing the revisionNumber
	[Tags]	Functional	certificate create	POST    current
    ${response}     run keyword and ignore error    Modify Certificate    Certificate/Modify_RegressionSchemeCertificate_withoutChangingRevisionNumber.json   ${Certificate_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "The Revision Number already exists for previous Certificate version"  Fail	test1 Teardown
	log to console  ${msg}
