...
10. Render Verdict
	[Tags]	Functional	POST	notcurrent
	${response1}	Render Verdict  group03/Test_Case_59/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_59/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group03/Test_Case_59/inputrequest/request3.json	${asset_Id_Product1}
...
14. Search Certificate with Certificate Type and Issuing Body and Mark and Owner Reference and Certificate Name
    [Tags]	Functional	notcurrent
    ${response}  Search Certificate   certificateType=Regression%20Scheme&issuingBody=USA&mark=Listed&ownerReference=${Cert_Owner_Ref}&certificateName=${Certificate_Name}
