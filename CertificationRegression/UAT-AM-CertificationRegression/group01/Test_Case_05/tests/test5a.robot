...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group01/Test_Case_05/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group01/Test_Case_05/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group01/Test_Case_05/inputrequest/request3.json	${asset_Id_Product1}
...
13. Associate parties to certificate
    [Tags]	Functional	certificate POST    current
    associate parties to certificate    Certificate/Associate_Parties_WithoutProduct_Certificate.json