...
10. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group02/Test_Case_38/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group02/Test_Case_38/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group02/Test_Case_38/inputrequest/request3.json	${asset_Id_Product1}
...
15. Certify certificate
    [Tags]	Functional	certificate	create	POST    current
    run keyword and ignore error  Certify Certificate  Certificate/Certify_RegressionCertificate_withIDequaltoFutureDate.json
