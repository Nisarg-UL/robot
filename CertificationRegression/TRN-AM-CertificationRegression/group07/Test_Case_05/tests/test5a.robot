...
13. Edit certificate using Invalid Date format
    [Tags]	Functional	certificate create	POST    current
    ${response}     run keyword and ignore error    Edit certificate  Certificate/EditOfRegressionSchemeCertificate_withInvalidDateFormat.json    ${Certificate_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Exception occurred while validating the certificate - Invalid date format Test Date -Data Type- Date. Valid format 'YYYY-MM-DD'"  Fail	test1 Teardown
	log to console  ${msg}
