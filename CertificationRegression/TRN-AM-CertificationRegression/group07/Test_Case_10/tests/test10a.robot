...
17. Modify Certificate by changing the certificate type
	[Tags]	Functional	certificate create	POST    current
    ${response}     run keyword and ignore error    Modify Certificate    Certificate/Modify_RegressionSchemeCertificate_changingCertificateType.json   ${Certificate_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Exception occurred while validating the certificate - Asset Data Validation -> No data matches with list of values. Error while validating Certificate Type List of values Regression Scheme"  Fail	test1 Teardown
	log to console  ${msg}