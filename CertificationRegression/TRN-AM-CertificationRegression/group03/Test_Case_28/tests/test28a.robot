...
10. Render Verdict
	[Tags]	Functional	POST	notcurrent
	${response1}	Render Verdict  group03/Test_Case_28/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_28/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group03/Test_Case_28/inputrequest/request3.json	${asset_Id_Product1}
...
14. Search Certificate with Certificate Type and Mark and Certificate Name
    [Tags]	Functional	notcurrent
    ${response}  Search Certificate   certificateType=Regression%20Scheme&mark=Listed&certificateName=${Certificate_Name}
