...
13. Lock Certificate with Certificate Parameters
    [Tags]	Functional	current
    ${response}  Lock Certificate  Lock_Template.json    certificateType=Regression%20Scheme&ownerReference=${Cert_Owner_Ref}&certificateName=${Certificate_Name}
...
13. UnLock Certificate with missing Certificate Parameters
    [Tags]	Functional	current
    ${response}  run keyword and ignore error  Unlock Certificate with Certificate Parameters  Unlock_Template.json    certificateType=Regression%20Scheme&certificateName=${Certificate_Name}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "'ownerReference' mandatory key missing in query parameter"  Fail	test1 Teardown
	log to console  ${msg}
