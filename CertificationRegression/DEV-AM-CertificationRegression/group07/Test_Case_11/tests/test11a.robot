...
17. Modify Certificate by changing the owner reference
	[Tags]	Functional	certificate create	POST    current
    ${response}     run keyword and ignore error    Modify Certificate with New Owner Reference    Certificate/Modify_RegressionSchemeCertificate_changingOwnerReference.json   ${Certificate_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Certificate Name, Certificate Type, Owner Reference cannot be modified"  Fail	test1 Teardown
	log to console  ${msg}