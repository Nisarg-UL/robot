...
19. Modify Certificate
	[Tags]	Functional	certificate create	POST    current
    Modify Certificate    Certificate/Modify_RegressionSchemeCertificate_ChangingDateValueEmpty.json   ${Certificate_Id}

20. Add parties to certificate
    [Tags]	Functional	certificate	create	POST    current
    Add Parties to Certificate    Certificate/Associate_Parties_Owner_Applicant_Reference_RegressionCertificate.json   ${Certificate_Id_Modify}
...
21. Certify certificate with Decisioning
	...
	run keyword if  "${msg}" != "Exception occurred while validating the certificate - Error while validating Test Date Value cannot be empty\/null"  Fail	test1 Teardown
	log to console  ${msg}

22. End Date CET_V2.1
	[Tags]	Functional	POST	current
