...
17. Modify Certificate using Wrong DataType
	[Tags]	Functional	certificate create	POST    current
    ${response}     run keyword and ignore error    Modify certificate  Certificate/EditOfRegressionSchemeCertificate_withWrongDataType.json    ${Certificate_Id}
    ${msg}  Get Error Message  ${response_api}
	run keyword if  "${msg}" != "Exception occurred while validating the certificate - Error while validating attribute- Energy Efficiency Rating -Data Type- decimal"  Fail	test1 Teardown
	log to console  ${msg}
