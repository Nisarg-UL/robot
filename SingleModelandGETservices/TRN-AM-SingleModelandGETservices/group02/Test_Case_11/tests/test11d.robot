...
14. View Private Label details without using UserId & Role
	[Tags]	Functional	certificate create	POST    current
	View Private Label details     ${PrivateLabel_Id}
    ${attr_name}	get_attribute_names  ${cert_attr}   name
    run keyword if  ${attr_name} != ['Certificate Type', 'Issuing Body', 'Mark', 'Certificate Name', 'Revision Number']     Fail
    run keyword if  ${pl_status} != "Under Revision"     Fail
    ${attr_name}	get_attribute_names  ${cert_attr}   displayName
    run keyword if  ${attr_name} != ['Certification Scheme', 'Certification Body', 'Mark', 'Certificate Name', 'Revision Number']     Fail
    run keyword if  ${pl_status} != "Under Revision"     Fail
