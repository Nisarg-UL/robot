...
15. View Private Label Asset details using UserId
	[Tags]	Functional	certificate create	POST    current
	${result}    View Private Label Assets using Certificate     ${Certificate_Id}   ${asset_Id_Product1}    user=81349
    run keyword if  "${result}" != "${asset_Id_Product1}"     Fail

16. Get Asset details using UserId
	[Tags]	Functional	certificate create	POST    current
	Get Asset Details using UserId     ${asset_Id_Product1}   81349
