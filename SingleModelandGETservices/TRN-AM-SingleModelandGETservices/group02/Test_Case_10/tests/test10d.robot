...
17. Get Projects and Decisions from Private Label without using UserID & Role
	[Tags]	Functional	certificate create	POST    current
    ${has_certify}   Get Projects and Decisions from Private Label    ${PrivateLabel_Id}    ${EMPTY}
    Should not be Empty     ${pl_decisions}
    ${attr_names}	privatelabel_certify_attributes  ${pl_decisions}    name
    ${result1}   compare_list_of_items    "${attr_names}"    "['Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']"
    should be equal  '${result1}'    'Lists are same'
    ${attr_display_names}	privatelabel_certify_attributes  ${pl_decisions}    displayName
    ${result2}   compare_list_of_items   "${attr_display_names}"    "['Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']"
