...
12. Get Certify from Certificate using UserID & Role
	[Tags]	Functional	certificate create	POST    current
    ${has_certify}   Get Certify from Certificate using UserID & Role    ${Certificate_Id}    ${user}   ${role}
    Should not be Empty     ${has_certify}
    ${certify}	get_questions_value  ${cert_certify}    name
    run keyword if  ${certify} != ['Issue Date', 'Revision Date', 'Withdrawal Date', 'Expiry Date', 'Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']     Fail
    ${certify}	get_questions_value  ${cert_certify}    displayName
    run keyword if  ${certify} != ['Date of Issue', 'Date of Revision', 'Date of Withdrawn', 'Date of Expiry', 'Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']     Fail
