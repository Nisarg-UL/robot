...
16. Certify Private Label
    [Tags]	Functional	certificate	PL  POST    current
    Add/unlink Project and Decisions to Private Label  Private_Label/Certify_PrivateLabelCertificate.json   false

17. Get Projects and Decisions from Private Label using UserID
	[Tags]	Functional	certificate create	POST    current
    ${has_certify}   Get Projects and Decisions from Private Label    ${PrivateLabel_Id}    user=81349
    Should not be Empty     ${pl_decisions}
    ${attr_names}	privatelabel_certify_attributes  ${pl_decisions}    name
    ${result1}   compare_list_of_items    "${attr_names}"    "['Issue Date', 'Revision Date', 'Withdrawal Date', 'Expiry Date', 'Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']"
    should be equal  '${result1}'    'Lists are same'
    ${attr_display_names}	privatelabel_certify_attributes  ${pl_decisions}    displayName
    ${result2}   compare_list_of_items   "${attr_display_names}"    "['Date of Issue', 'Date of Revision', 'Date of Withdrawn', 'Date of Expiry', 'Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']"
    should be equal  '${result2}'    'Lists are same'
