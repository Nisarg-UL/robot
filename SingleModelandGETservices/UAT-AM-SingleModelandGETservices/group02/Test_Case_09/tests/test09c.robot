...
12. Get Certify from Certificate using Role
	[Tags]	Functional	certificate create	POST    current
    ${has_certify}   Get Certify from Certificate using Role    ${Certificate_Id}    ${role}
    Should not be Empty     ${has_certify}
    ${certify}	get_questions_value  ${cert_certify}    name
    run keyword if  ${certify} != ['Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']     Fail
