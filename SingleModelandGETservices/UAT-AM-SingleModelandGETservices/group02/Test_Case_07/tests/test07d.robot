...
13. Certificate Audit without using UserId & Role
    [Tags]	Functional	current
    ${response}  Certificate Audit  certificateId=${Certificate_Id}
    ${attr_names}	projectHistory_certify_attributes  ${response}    name
    run keyword if  ${attr_names} != ['Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']     Fail
    ${attr_display_names}	projectHistory_certify_attributes  ${response}    displayName
    run keyword if  ${attr_display_names} != ['Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']    Fail
