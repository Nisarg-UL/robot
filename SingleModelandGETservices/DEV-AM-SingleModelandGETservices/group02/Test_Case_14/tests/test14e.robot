...
17. View Private Label Certify Details using UserID & Role
	[Tags]	Functional	certificate create	POST    current
    View Private Label Certify using Parameters    ${PrivateLabel_Id}    user=81349&role=Public
    ${attr_names}	privatelabel_certify  ${pl_certify}    name
    should be equal    "${attr_names}"    "['Issue Date', 'Revision Date', 'Expiry Date', 'Withdrawal Date', 'Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']"
    ${attr_display_names}	privatelabel_certify  ${pl_certify}    displayName
    should be equal   "${attr_display_names}"    "['Date of Issue', 'Date of Revision', 'Date of Expiry', 'Date of Withdrawn', 'Certification Comment', 'Certifier Employee Name', 'Certifier Employee Number']"
