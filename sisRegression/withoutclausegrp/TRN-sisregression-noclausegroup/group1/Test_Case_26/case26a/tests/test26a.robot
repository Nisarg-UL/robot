...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group1/Test_Case_26/case26a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group1/Test_Case_26/case26a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group1/Test_Case_26/case26a/inputrequest/request3.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '26, S3 : PASS'	Fail	test1 Teardown
	${result1}  clause text  ${asset_Id_Product1}
	log to console	'Result = ${result} on Cl. ${result1}