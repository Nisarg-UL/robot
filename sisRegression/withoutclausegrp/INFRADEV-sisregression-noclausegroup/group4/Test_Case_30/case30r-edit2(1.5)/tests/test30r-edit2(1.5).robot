...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	saverequirement_group4_subgrp4.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group4/Test_Case_30/case30r-edit2(1.5)/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group4/Test_Case_30/case30r-edit2(1.5)/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group4/Test_Case_30/case30r-edit2(1.5)/inputrequest/request3.json	${asset_Id_Product1}
	...
	run keyword if	${result} != ['1.5 : NA', '1.6 : PASS']   Fail	test1 Teardown
	log to console	Result Before Edit - ${result}
	${response4}	Render Verdict	group4/Test_Case_30/case30r-edit2(1.5)/inputrequest/request4.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	${result}	Evaluation Summary	${asset_Id_Product1}
	run keyword if	${result} != ['1.5 : FAIL', '1.6 : PASS']   Fail	test1 Teardown
