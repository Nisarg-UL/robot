...
2. Asset Creation With POST Request
    ...
    create product2_siscase2 asset  CreationOfRegressionProduct2_siscase3.json
...
19. Link Product2 & Product3 to Product 1
    [Tags]	Functional	current
	${file}=	Get File	input/Link_Product2&Product3toProduct1_FU1_withoutLinkageDetails.json
	${JSON}=	replace variables	${file}
	${result}=	Post Data To Endpoint	/assets/createAssetDetails	${JSON}	200
	set global variable	${assetLinkSeqId}	${result.json()["data"]["hasComponents"][0]["assetAssetLinkSeqId"]}
	log to console	${assetLinkSeqId}
...
23. Requirement Assignment To Product
	...
	Save Requirement	saverequirement_group3_29.json	${asset_Id_Product1}
...
25. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group3/Test_Case_29/case29c/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group3/Test_Case_29/case29c/inputrequest/request2.json	${asset_Id_Product1}
...
26. Complete Evaluation
	...
	run keyword if	'${result}' != '29 : FAIL'	Fail	test1 Teardown
