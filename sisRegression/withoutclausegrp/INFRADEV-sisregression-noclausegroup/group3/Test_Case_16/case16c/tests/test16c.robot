...
3. Asset Creation With POST Request
    ...
    create product3 asset	CreationOfRegressionProduct3_siscase3.json
...
12. Link Product2 to product3
    [Tags]	Functional	current
	${file}=	Get File	input/Link_Product2toProduct3_FU1_withoutLinkageDetails.json
...
19. Link Product2 & Product3 to Product 1
    [Tags]	Functional	current
	${file}=	Get File	input/Link_Product2&Product3toProduct1_FU2_withoutLinkageDetails.json
	${JSON}=	replace variables	${file}
	${result}=	Post Data To Endpoint	/assets/createAssetDetails	${JSON}	200
	set global variable	${assetLinkSeqId1}	${result.json()["data"]["hasComponents"][0]["assetAssetLinkSeqId"]}
	log to console	${assetLinkSeqId1}
	set global variable	${assetLinkSeqId2}	${result.json()["data"]["hasComponents"][1]["assetAssetLinkSeqId"]}
...
25. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict for 202	group3/Test_Case_16/case16c/inputrequest/request1.json	${asset_Id_Product1}
#	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group3/Test_Case_16/case16c/inputrequest/request2.json	${asset_Id_Product1}
...
26. Complete Evaluation
	...
	run keyword if	'${result}' != '16 : NA'	Fail	test1 Teardown
