...
def extract_and_replace_random_project_no(blob):
	...
	data_dict = data['data']
	global project_no
	project_no = randint(10000000000, 99999999999)
	asset_list = data_dict['asset'][0]
...
def extract_and_replace_date_for_certificate_scheme(blob):
	...
	attrib_dict = cert_list['attributes'][14]
	for key, value in attrib_dict.items():
		if value == "Replace Date":
			attrib_dict[key] = Today_Date
	return json.dumps({"data": data_dict})
...
def extract_and_replace_issue_date_for_certificate(blob):
	data = json.loads(blob)
	data_dict = data['data']
	global Today_Date
	Today_Date = time.strftime('%Y-%m-%d')
...
def extract_and_replace_issue_date_and_expiry_date_for_certificate(blob):
	data = json.loads(blob)
	data_dict = data['data']
	global Today_Date
	Today_Date = time.strftime('%Y-%m-%d')
	...
	for key, value in attrib_dict.items():
		if value == "Replace Date":
			attrib_dict[key] = Today_Date
	attrib_dict1 = cert_list['attributes'][3]
...
def extract_and_replace_issue_date_and_withdrawal_date_for_certificate(blob):
	data = json.loads(blob)
	data_dict = data['data']
	global Today_Date
	Today_Date = time.strftime('%Y-%m-%d')
	...
	for key, value in attrib_dict.items():
		if value == "Replace Date":
			attrib_dict[key] = Today_Date
	attrib_dict1 = cert_list['attributes'][2]
	for key, value in attrib_dict1.items():
		if value == "Replace Date":
			attrib_dict1[key] = Today_Date
	return json.dumps({"data": data_dict})
...
def extract_and_replace_issue_date_for_private_label_scheme(blob):
	...
	global Today_Date
	Today_Date = time.strftime('%Y-%m-%d')
	pl_dict = data_dict['privateLabel'][0]
