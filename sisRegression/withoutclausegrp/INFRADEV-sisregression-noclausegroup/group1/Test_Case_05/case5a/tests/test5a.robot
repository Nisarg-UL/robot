...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group1/Test_Case_05/case5a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group1/Test_Case_05/case5a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group1/Test_Case_05/case5a/inputrequest/request3.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '5, R1 : PASS'	Fail	test1 Teardown
	${result1}	Clause ID  ${asset_Id_Product1}
	log to console	Result - ${result} at clause ${result1} (tested numeric entry, less than equal to operator, implicit pass)