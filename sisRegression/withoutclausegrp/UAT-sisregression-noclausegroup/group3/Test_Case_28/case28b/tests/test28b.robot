...
#    [Tags]	Functional	current
#	${file}=	Get File	input/Link_Product2toProduct3_FU2_withoutLinkageDetails.json
...
25. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group3/Test_Case_28/case28b/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group3/Test_Case_28/case28b/inputrequest/request2.json	${asset_Id_Product1}
...
26. Complete Evaluation
	...
	run keyword if	'${result}' != '28, U2 : TEST REQUIRED'	Fail	test1 Teardown
