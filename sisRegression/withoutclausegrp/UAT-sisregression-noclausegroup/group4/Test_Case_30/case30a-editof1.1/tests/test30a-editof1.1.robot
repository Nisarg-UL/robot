...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group4/Test_Case_30/case30a-editof1.1/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group4/Test_Case_30/case30a-editof1.1/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group4/Test_Case_30/case30a-editof1.1/inputrequest/request3.json	${asset_Id_Product1}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${response4}	Render Verdict  group4/Test_Case_30/case30a-editof1.1/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response4.status_code} != 200	Fail	test1 Teardown
	${response5}	Render Verdict	group4/Test_Case_30/case30a-editof1.1/inputrequest/request5.json	${asset_Id_Product1}
	${clause}   has more clauses  ${response5.text}
    run keyword if  '${clause}' != 'False'  fatal error
    ${result}	Evaluation Summary	${asset_Id_Product1}
	run keyword if	${result} != ['1.1 : PASS', '1.2 : PASS', '1.3 : PASS', '1.4 : PASS']   Fail	test1 Teardown
	log to console	Result Before Edit - ${result}
	${response6}	Render Verdict  group4/Test_Case_30/case30a-editof1.1/inputrequest/request6.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	run keyword if  '${clause}' != 'False'  fatal error
    ${result}	Evaluation Summary	${asset_Id_Product1}
