...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group4/Test_Case_30/case30f/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group4/Test_Case_30/case30f/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group4/Test_Case_30/case30f/inputrequest/request3.json	${asset_Id_Product1}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${response4}	Render Verdict  group4/Test_Case_30/case30f/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response4.status_code} != 200	Fail	test1 Teardown
	${response5}	Render Verdict	group4/Test_Case_30/case30f/inputrequest/request5.json	${asset_Id_Product1}
