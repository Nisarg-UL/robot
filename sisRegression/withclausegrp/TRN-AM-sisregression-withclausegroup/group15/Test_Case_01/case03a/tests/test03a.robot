...
8. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group15/Test_Case_01/case03a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group15/Test_Case_01/case03a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group15/Test_Case_01/case03a/inputrequest/request3.json	${asset_Id_Product1}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${response4}	Render Verdict	group15/Test_Case_01/case03a/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response4.status_code} != 200	Fail	test1 Teardown
	${response5}	Render Verdict	group15/Test_Case_01/case03a/inputrequest/request5.json	${asset_Id_Product1}
	run keyword if	${response5.status_code} != 200	Fail	test1 Teardown
	${response6}	Render Verdict	group15/Test_Case_01/case03a/inputrequest/request6.json	${asset_Id_Product1}
...
9. Complete Evaluation
	...
	run keyword if	${result} != ['1 : NA : Clause Group = ', '2 : NA : Clause Group = ', '3 : NA : Clause Group = ', '4 : NA : Clause Group = ', 'Rollup : NA : Clause Group = ']    Fail	test1 Teardown
