...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	saverequirement_group1_07.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group01/Test_Case_07/case7a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group01/Test_Case_07/case7a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '7 : ALTERNATE EVALUATION OPTION : Clause Group = g'	Fail	test1 Teardown
	${result1}	Clause Text  ${asset_Id_Product1}
	${result2}	AEO details	${asset_Id_Product1}
	log to console	Result - ${result} to Cl. ${result1} also have AEO Details = ${result2}