...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group01/Test_Case_02/case2a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group01/Test_Case_02/case2a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	log to console	Result - ${result} , Clause ${result1} (implicit NA)