...
16. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_02/case2b/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group05/Test_Case_02/case2b/inputrequest/request2.json	${asset_Id_Product1}
...
17. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	set global variable    ${assessmentId}    ${assessmentId1}
	Save Requirement	saverequirement_group1_08_withcomponent1context2.json	${asset_Id_Product1}

18. Get Assessment_ParamId
	[Tags]	Functional	GET	current
...
19. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_02/case2c/inputrequest/request3.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group05/Test_Case_02/case2c/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response2.text}
    run keyword if  '${clause}' != 'False'  fatal error

20. Complete Evaluation
	[Tags]	Functional	POST	current
...
23. Get Context
    ...
    ${Context_desc1}   Get Context Description  ${response}  ${asset_Id_Product2}   ${assessmentParamId1}
    run keyword if  '${Context_desc1}' != 'Automation_component1_context2'   Fail	test1 Teardown
    ${Asset_linkages1}   Get Asset Linkages  ${response}   ${asset_Id_Product2}  ${assessmentParamId1}
