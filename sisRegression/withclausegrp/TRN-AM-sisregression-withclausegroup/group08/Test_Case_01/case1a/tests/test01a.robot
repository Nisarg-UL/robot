...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group08/Test_Case_01/case1a/inputrequest/request1.json	${asset_Id_Product1}
	...
	${response2}	Render Verdict	group08/Test_Case_01/case1a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${result}  Get Guidance Notes AP2   ${response_api}
	run keyword if	'${result}' != 'GN on AP1 - row 2.'	Fail	test1 Teardown
    log to console  ${result}
	${response3}	Render Verdict	group08/Test_Case_01/case1a/inputrequest/request3.json	${asset_Id_Product1}
