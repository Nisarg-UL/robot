...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	saverequirement_group8_03.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group08/Test_Case_10/case10a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
#    ${result}  Get Guidance Notes AP1_2   ${response_api}
#	run keyword if	'${result}' != 'GN on AP2 - row 6.'	Fail	test1 Teardown
#	log to console  ${result}
	${result}  Get Guidance Notes RP1   ${response_api}
	run keyword if	'${result}' != 'GN on RP1 - row 10.'	Fail	test1 Teardown
	log to console  ${result}
	${response2}	Render Verdict	group08/Test_Case_10/case10a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '3.1 : PASS : Clause Group = c'	Fail	test1 Teardown
