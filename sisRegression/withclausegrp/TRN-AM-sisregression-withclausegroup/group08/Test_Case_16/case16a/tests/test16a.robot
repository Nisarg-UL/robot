...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	saverequirement_group8_05.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group08/Test_Case_16/case16a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${result}  Get Guidance Notes AP1   ${response_api}
	run keyword if	'${result}' != 'GN on AP5 inequality - row 17.'	Fail	test1 Teardown
	log to console  ${result}
	${response2}	Render Verdict	group08/Test_Case_16/case16a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '5.1 : PASS : Clause Group = e'	Fail	test1 Teardown
