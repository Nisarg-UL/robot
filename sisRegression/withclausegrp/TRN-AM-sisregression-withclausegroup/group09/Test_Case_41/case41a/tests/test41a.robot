...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	Saverequirement_group9/saverequirement_group9_041.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group09/Test_Case_41/case41a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${result}  Get Guidance Notes AP1   ${response_api}
    run keyword if	"${result}" != "GN special character check for character Š"	Fail	test1 Teardown
    log to console  ${result}
	${response2}	Render Verdict	group09/Test_Case_41/case41a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '41 : PASS : Clause Group = a41'	Fail	test1 Teardown
