...
8. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict	group10/Test_Case_02/case02c/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group10/Test_Case_02/case02c/inputrequest/request2.json	${asset_Id_Product1}
...
9. Complete Evaluation
	...
	run keyword if	'${result}' != 'a-40to30 : PASS : Clause Group = a'	Fail	test1 Teardown
