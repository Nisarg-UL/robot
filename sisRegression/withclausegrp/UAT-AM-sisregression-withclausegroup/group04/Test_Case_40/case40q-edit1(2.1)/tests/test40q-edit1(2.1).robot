...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group04/Test_Case_40/case40q-edit1(2.1)/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_40/case40q-edit1(2.1)/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_40/case40q-edit1(2.1)/inputrequest/request3.json	${asset_Id_Product1}
	...
	run keyword if	${result} != ['2.1 : PASS : Clause Group = i,j,k', '2.4 : PASS : Clause Group = l,m,n']   Fail	test1 Teardown
	log to console	Result Before Edit - ${result}
    ${response4}	Render Verdict	group04/Test_Case_40/case40q-edit1(2.1)/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response4.status_code} != 200	Fail	test1 Teardown
	${response5}	Render Verdict	group04/Test_Case_40/case40q-edit1(2.1)/inputrequest/request5.json	${asset_Id_Product1}
	run keyword if	${response5.status_code} != 200	Fail	test1 Teardown
	${response6}	Render Verdict	group04/Test_Case_40/case40q-edit1(2.1)/inputrequest/request6.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	${result}	Evaluation Summary	${asset_Id_Product1}
	run keyword if	${result} != ['2.1 : PASS : Clause Group = i,j,k', '2.4 : NA : Clause Group = l', '2.6 : NA : Clause Group = m', '2.7 : NA : Clause Group = n']   Fail	test1 Teardown
