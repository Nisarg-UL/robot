...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group04/Test_Case_40/case40k-edit1.1d,1.3b/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_40/case40k-edit1.1d,1.3b/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_40/case40k-edit1.1d,1.3b/inputrequest/request3.json	${asset_Id_Product1}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${response4}	Render Verdict  group04/Test_Case_40/case40k-edit1.1d,1.3b/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response4.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response4.text}
    run keyword if  '${clause}' != 'False'  fatal error
	${result}	Evaluation Summary	${asset_Id_Product1}
	run keyword if	${result} != ['1.1d,1.3b : PASS : Clause Group = e,g', '1.2d : PASS : Clause Group = f', '1.4b : PASS : Clause Group = h']   Fail	test1 Teardown
	log to console	Result Before Edit - ${result}
	${response5}	Render Verdict	group04/Test_Case_40/case40k-edit1.1d,1.3b/inputrequest/request5.json	${asset_Id_Product1}
	run keyword if	${response5.status_code} != 200	Fail	test1 Teardown
	${response6}	Render Verdict	group04/Test_Case_40/case40k-edit1.1d,1.3b/inputrequest/request6.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	${result}	Evaluation Summary	${asset_Id_Product1}
	run keyword if	${result} != ['1.1e, 1.2c : PASS : Clause Group = e,f', '1.3c : PASS : Clause Group = g', '1.4b : PASS : Clause Group = h']   Fail	test1 Teardown
