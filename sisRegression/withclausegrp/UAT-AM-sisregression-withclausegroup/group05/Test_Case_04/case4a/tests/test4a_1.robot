...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_04/case4a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group05/Test_Case_04/case4a/inputrequest/request2.json	${asset_Id_Product1}
...
12. Get Context
    ...
    run keyword if  len(${Asset_linkages}) != 0   Fail	test1 Teardown
