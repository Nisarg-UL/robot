...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group01/Test_Case_10/case10b/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group01/Test_Case_10/case10b/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '10 : NA : Clause Group = j'	Fail	test1 Teardown
	${result1}	Clause ID  ${asset_Id_Product1}
	log to console	'Result = ${result} at Cl. ${result1} tested multiple parameter and implicit NA