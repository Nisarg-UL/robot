...
6. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	SaveRequirement_group10-15/saverequirement_group12_31.json	${asset_Id_Product1}
...
8. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group12/Test_Case_31/case31a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group12/Test_Case_31/case31a/inputrequest/request2.json	${asset_Id_Product1}
...
9. Complete Evaluation
	...
	run keyword if	'${result}' != '31 : PASS : Clause Group = 31'	Fail	test1 Teardown
