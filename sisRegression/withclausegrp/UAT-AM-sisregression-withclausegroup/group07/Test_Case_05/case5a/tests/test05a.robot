...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group01/Test_Case_01/case1a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group01/Test_Case_01/case1a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group01/Test_Case_01/case1a/inputrequest/request3.json	${asset_Id_Product1}
...
11. Standard Assignment To Product (Product Evaluation Set Up) - Edit Evaluation Scope
	[Tags]	Functional	POST	current
	standard assignment	productevaluationsetup_withexact2000character_Eval_comments.json	${asset_Id_Product1}
