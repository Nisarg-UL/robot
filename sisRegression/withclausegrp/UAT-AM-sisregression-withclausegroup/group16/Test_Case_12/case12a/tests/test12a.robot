...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group16/Test_Case_12/case12a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group16/Test_Case_12/case12a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != 'a-less100 : PASS : Clause Group = a'	Fail	test1 Teardown
	log to console	'Result = ${result}

12. Get Reviewer Summary Details
    [Tags]	Functional	GET	current
	${response}  Get Reviewer Summary Details    ${asset_Id_Product1}    ${assessmentId}
	run keyword if	${response[0]} != "a-less100"  Fail	test1 Teardown
    run keyword if	${response[1]} != "Group 10 - Non-Data Linked Scenarios"  Fail	test1 Teardown
    run keyword if	${response[2]} != ""  Fail	test1 Teardown
    run keyword if	${response[3]} != "Test Case 02 - Selecting Number within Ranges, Negative"  Fail	test1 Teardown
