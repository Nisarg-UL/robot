...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	SaveRequirement_group10-15/saverequirement_group11_08.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group11/Test_Case_08/case08a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group11/Test_Case_08/case08a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '7 : PASS : Clause Group = 7'	Fail	test1 Teardown
