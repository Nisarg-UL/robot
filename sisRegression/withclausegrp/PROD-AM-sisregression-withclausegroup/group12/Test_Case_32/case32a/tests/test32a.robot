...
8. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group12/Test_Case_32/case32a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group12/Test_Case_32/case32a/inputrequest/request2.json	${asset_Id_Product1}
...
9. Complete Evaluation
	...
	run keyword if	'${result}' != '32 : PASS : Clause Group = 32'	Fail	test1 Teardown
