...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	saverequirement_group8_01.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group08/Test_Case_02/case2a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
    ${result}  Get Guidance Notes AP1   ${response_api}
	run keyword if	'${result}' != 'GN on AP1 - row 2.'	Fail	test1 Teardown
	${response2}	Render Verdict	group08/Test_Case_02/case2a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${result}  Get Guidance Notes AP2   ${response_api}
	run keyword if	'${result}' != 'GN on AP1 - row 2.'	Fail	test1 Teardown
    log to console  ${result}
#    ${result}  Get Guidance Notes RP1   ${response_api}
#    run keyword if	'${result}' != 'GN on RP1.'	Fail	test1 Teardown
#    log to console  ${result}
	${response3}	Render Verdict	group08/Test_Case_02/case2a/inputrequest/request3.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '1.1 : PASS : Clause Group = a'	Fail	test1 Teardown
