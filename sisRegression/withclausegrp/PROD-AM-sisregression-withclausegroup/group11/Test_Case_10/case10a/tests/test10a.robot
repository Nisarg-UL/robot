...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group11/Test_Case_10/case10a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group11/Test_Case_10/case10a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '9 : PASS : Clause Group = 9'	Fail	test1 Teardown
