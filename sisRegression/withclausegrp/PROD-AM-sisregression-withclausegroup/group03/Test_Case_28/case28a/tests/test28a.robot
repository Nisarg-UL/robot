...
26. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group03/Test_Case_28/case28a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_28/case28a/inputrequest/request2.json	${asset_Id_Product1}
...
27. Complete Evaluation
	...
	run keyword if	'${result}' != '28, U1 : ALTERNATE EVALUATION OPTION : Clause Group = k'	Fail	test1 Teardown
	${result1}  AEO details  ${asset_Id_Product1}
	log to console	'Result = ${result} with AEO details = "${result1}"