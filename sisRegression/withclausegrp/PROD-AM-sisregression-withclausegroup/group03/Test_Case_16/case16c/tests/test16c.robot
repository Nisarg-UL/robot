...
25. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group03/Test_Case_16/case16c/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_16/case16c/inputrequest/request2.json	${asset_Id_Product1}
...
26. Complete Evaluation
	...
	run keyword if	'${result}' != '16 : NA : Clause Group = a'	Fail	test1 Teardown
