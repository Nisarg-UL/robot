...
21. Requirement Assignment To Product
	...
	Save Requirement	saverequirement_group3_27_withproduct1component1.json	${asset_Id_Product1}
...
23. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_03/case3d/inputrequest/request1.json	${asset_Id_Product1}
...
24. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	set global variable    ${assessmentId}    ${assessmentId1}
	Save Requirement	saverequirement_group3_27_withproduct1component2.json	${asset_Id_Product1}

25. Get Assessment_ParamId
	[Tags]	Functional	GET	current
...
26. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_03/case3d/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response1.text}
    run keyword if  '${clause}' != 'False'  fatal error

27. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	set global variable    ${assessmentId}    ${assessmentId1}
	Save Requirement	saverequirement_group3_27_withproduct1.json 	${asset_Id_Product1}

28. Get Assessment_ParamId
	[Tags]	Functional	GET	current
...
29. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group05/Test_Case_03/case3d/inputrequest/request3.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${clause}   has more clauses  ${response1.text}
    run keyword if  '${clause}' != 'False'  fatal error

30. Edit Product1 Attributes
    [Tags]	Functional	asset	create	POST    current
    Edit Product1 Asset	EditOfTp1Att8RegressionProduct1_siscase1_withCol_ID.json    ${asset_Id_Product1}
...
32. Get Context
    ...
    run keyword if  '${Eval_clauses1}' != 'True'   Fail	test1 Teardown
    ${Eval_clauses2}   Get Evaluated Clauses  ${response}    ${assessmentParamId2}
    run keyword if  '${Eval_clauses2}' != 'True'   Fail	test1 Teardown
    ...
    run keyword if  '${Verdict_render1}' != 'True'   Fail	test1 Teardown
    ${Verdict_render2}   Get Verdict Rendered  ${response}   ${assessmentParamId2}
    run keyword if  '${Verdict_render2}' != 'True'   Fail	test1 Teardown