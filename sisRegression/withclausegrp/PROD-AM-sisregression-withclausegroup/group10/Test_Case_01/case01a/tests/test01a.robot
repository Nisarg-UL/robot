...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	saverequirement_group1_10.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group10/Test_Case_01/case01a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group10/Test_Case_01/case01a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '10 : FAIL : Clause Group = j'	Fail	test1 Teardown
	${result1}	Clause Text  ${asset_Id_Product1}
	log to console	'Result = ${result} at cl. ${result1} tested multiple applicability parameters and implicit pass