...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group04/Test_Case_40/case40c-edit1.1-1.2-1.3/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_40/case40c-edit1.1-1.2-1.3/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_40/case40c-edit1.1-1.2-1.3/inputrequest/request3.json	${asset_Id_Product1}
	...
	run keyword if	${result} != ['1.1, 1.2, 1.3 : PASS : Clause Group = a,b,c', '1.4 : PASS : Clause Group = d']   Fail	test1 Teardown
	log to console	Result Before Edit - ${result}
	${response4}	Render Verdict  group04/Test_Case_40/case40c-edit1.1-1.2-1.3/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response4.status_code} != 200	Fail	test1 Teardown
	${response5}	Render Verdict	group04/Test_Case_40/case40c-edit1.1-1.2-1.3/inputrequest/request5.json	${asset_Id_Product1}
	run keyword if	${response5.status_code} != 200	Fail	test1 Teardown
	${response6}	Render Verdict	group04/Test_Case_40/case40c-edit1.1-1.2-1.3/inputrequest/request6.json	${asset_Id_Product1}
