...
8. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group14/Test_Case_01/case01b/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group14/Test_Case_01/case01b/inputrequest/request2.json	${asset_Id_Product1}
...
9. Complete Evaluation
	...
	run keyword if	'${result}' != 'a1 : NA : Clause Group = a'	Fail	test1 Teardown
