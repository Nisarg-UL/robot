...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	saverequirement_group8_08.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group08/Test_Case_23/case23a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${result}  Get Guidance Notes AP1   ${response_api}
	run keyword if	'${result}' != 'GN on AP8 - row 23'	Fail	test1 Teardown
	log to console  ${result}
	${response2}	Render Verdict	group08/Test_Case_23/case23a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '8 : ALTERNATE EVALUATION OPTION : Clause Group = h'	Fail	test1 Teardown
