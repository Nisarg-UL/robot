...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group16/Test_Case_13/case13a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group16/Test_Case_13/case13a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group16/Test_Case_13/case13a/inputrequest/request3.json	${asset_Id_Product1}
...
10. Requirement Assignment To Product
	[Tags]	Functional	POST	current
    Save Requirement	SaveRequirement_group10-15/saverequirement_group10_02.json	${asset_Id_Product1}

11. Get Assessment_ParamId
	[Tags]	Functional	GET	current
	Get Assesment_ParamID_2onward  ${asset_Id_Product1}     Group 10 - Non-Data Linked Scenarios    Test Case 02 - Selecting Number within Ranges, Negative

12. Render Verdict
	[Tags]	Functional	POST	current
    ${response1}	Render Verdict  group16/Test_Case_13/case13a/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group16/Test_Case_13/case13a/inputrequest/request5.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
...
12. Get Reviewer Summary Details
    ...
    run keyword if	${response[9]} != "a-less100"  Fail	test1 Teardown
    run keyword if	${response[10]} != "Group 10 - Non-Data Linked Scenarios"  Fail	test1 Teardown
    run keyword if	${response[11]} != ""  Fail	test1 Teardown
    run keyword if	${response[12]} != "Test Case 02 - Selecting Number within Ranges, Negative"  Fail	test1 Teardown
    run keyword if	${response[13]} != ""  Fail	test1 Teardown
    run keyword if	${response[14]} != "PASS"  Fail	test1 Teardown