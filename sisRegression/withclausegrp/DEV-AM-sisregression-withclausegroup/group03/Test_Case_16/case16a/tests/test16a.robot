...
25. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group03/Test_Case_16/case16a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200    Fail 	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_16/case16a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group03/Test_Case_16/case16a/inputrequest/request3.json	${asset_Id_Product1}
...
26. Complete Evaluation
	...
	run keyword if	'${result}' != '16 : PASS : Clause Group = a'	Fail	test1 Teardown
