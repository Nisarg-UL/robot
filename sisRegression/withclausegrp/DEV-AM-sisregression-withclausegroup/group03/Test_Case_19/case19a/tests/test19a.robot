...
25. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group03/Test_Case_19/case19a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group03/Test_Case_19/case19a/inputrequest/request2.json	${asset_Id_Product1}
...
26. Complete Evaluation
	...
	run keyword if	'${result}' != '19 : PASS : Clause Group = d'	Fail	test1 Teardown
