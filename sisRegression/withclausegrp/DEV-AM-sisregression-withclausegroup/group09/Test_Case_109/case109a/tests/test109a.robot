...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	current
	Save Requirement	Saverequirement_group9/saverequirement_group9_109.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group09/Test_Case_109/case109a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200 	Fail	test1 Teardown
	${result}  Get Guidance Notes AP1   ${response_api}
    run keyword if	'${result}' != 'GN special character check for character Ô'	Fail	test1 Teardown
    log to console  ${result}
	${response2}	Render Verdict	group09/Test_Case_109/case109a/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '109 : PASS : Clause Group = a109'	Fail	test1 Teardown
