...
7. Requirement Assignment To Product
	[Tags]	Functional	POST	Notcurrent
	Save Requirement	SaveRequirement_group10-15/saverequirement_group11_16.json	${asset_Id_Product1}
...
9. Render Verdict
	[Tags]	Functional	POST	Notcurrent
	${response1}	Render Verdict	group11/Test_Case_16/case16c/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group11/Test_Case_16/case16c/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '15 : FAIL : Clause Group = 15'	Fail	test1 Teardown
