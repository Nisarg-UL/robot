...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group04/Test_Case_40/case40i-edit1.1b,1.2a/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_40/case40i-edit1.1b,1.2a/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_40/case40i-edit1.1b,1.2a/inputrequest/request3.json	${asset_Id_Product1}
	...
	run keyword if	${result} != ['1.1b, 1.2a : PASS : Clause Group = e,f', '1.3d, 1.4a : PASS : Clause Group = g,h']   Fail	test1 Teardown
	${response4}	Render Verdict  group04/Test_Case_40/case40i-edit1.1b,1.2a/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response4.status_code} != 200	Fail	test1 Teardown
	${response5}	Render Verdict	group04/Test_Case_40/case40i-edit1.1b,1.2a/inputrequest/request5.json	${asset_Id_Product1}
