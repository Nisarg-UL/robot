...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict  group04/Test_Case_40/case40n-edit1.3c/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group04/Test_Case_40/case40n-edit1.3c/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group04/Test_Case_40/case40n-edit1.3c/inputrequest/request3.json	${asset_Id_Product1}
	run keyword if	${response3.status_code} != 200	Fail	test1 Teardown
	${response4}	Render Verdict  group04/Test_Case_40/case40n-edit1.3c/inputrequest/request4.json	${asset_Id_Product1}
	run keyword if	${response4.status_code} != 200	Fail	test1 Teardown
	${response5}	Render Verdict	group04/Test_Case_40/case40n-edit1.3c/inputrequest/request5.json	${asset_Id_Product1}
    ...
    ${response6}	Render Verdict	group04/Test_Case_40/case40n-edit1.3c/inputrequest/request6.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	${result}	Evaluation Summary	${asset_Id_Product1}
	run keyword if	${result} != ['1.1a : FAIL : Clause Group = e', '1.2d : FAIL : Clause Group = f', '1.3d, 1.4a : PASS : Clause Group = g,h']   Fail	test1 Teardown
