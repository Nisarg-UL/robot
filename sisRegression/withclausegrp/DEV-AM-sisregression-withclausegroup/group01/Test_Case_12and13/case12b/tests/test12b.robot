...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group01/Test_Case_12and13/case12b/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group01/Test_Case_12and13/case12b/inputrequest/request2.json	${asset_Id_Product1}
	run keyword if	${response2.status_code} != 200	Fail	test1 Teardown
	${response3}	Render Verdict	group01/Test_Case_12and13/case12b/inputrequest/request3.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '12 : FAIL : Clause Group = l'	Fail	test1 Teardown
	${result1}	Clause ID  ${asset_Id_Product1}
	log to console	'Result = ${result} at Clause ${result1} only. (tests multiple clauses on implicit fail)