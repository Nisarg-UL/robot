...
9. Render Verdict
	[Tags]	Functional	POST	current
	${response1}	Render Verdict	group01/Test_Case_04/case4b/inputrequest/request1.json	${asset_Id_Product1}
	run keyword if	${response1.status_code} != 200	Fail	test1 Teardown
	${response2}	Render Verdict	group01/Test_Case_04/case4b/inputrequest/request2.json	${asset_Id_Product1}
...
10. Complete Evaluation
	...
	run keyword if	'${result}' != '4 : FAIL : Clause Group = d'	Fail	test1 Teardown
	${result1}	Clause ID  ${asset_Id_Product1}
	log to console	Result - ${result} at clause ${result1} (tested numeric entry, greater than equal to operator, explicit fail)
